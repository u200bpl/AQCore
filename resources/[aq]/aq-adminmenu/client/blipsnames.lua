...
Citizen.CreateThread(function()
    while true do
        Wait(1000)
        if ShowBlips then
            TriggerServerEvent('aq-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('aq-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        AQCore.Functions.Notify("Blips activated", "success")
    else
        ShowBlips = false
        AQCore.Functions.Notify("Blips deactivated", "error")
    end
end)

RegisterNetEvent('aq-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        AQCore.Functions.Notify("Names activated", "success")
    else
        ShowNames = false
        AQCore.Functions.Notify("Names deactivated", "error")
    end
end)

RegisterNetEvent('aq-admin:client:Show', function(players)
