...
RegisterNetEvent('aq-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('aq-weathersync:server:setWeather', selection.value)
            AQCore.Functions.Notify('Weather Changed To: '..selection.label)
        end})
    end
end)
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords' then
        ...
        AQCore.Functions.Notify("Coordinates copied to clipboard!", "success")
    elseif dataType == 'heading' then
        ...
        AQCore.Functions.Notify("Heading copied to clipboard!", "success")
    end
end
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        local menu_button10 = menu14:AddButton({
             ...
             select = function(btn)
                 TriggerServerEvent('AQCore:CallCommand', "car", { k })
             end
        })
    end
end
...
menu12_button2:On('Select', function(item)
    TriggerServerEvent('AQCore:CallCommand', "fix", {})
end)

menu12_button3:On('Select', function(item)
    TriggerServerEvent('AQCore:CallCommand', "admincar", {})
end)

menu12_button4:On('Select', function(item)
    TriggerServerEvent('AQCore:CallCommand', "dv", {})
end)

names_button:On('change', function()           
    TriggerEvent('aq-admin:client:toggleNames')
end)                                           
blips_button:On('change', function()           
    TriggerEvent('aq-admin:client:toggleBlips')
end)
    ...
    for k, v in ipairs(elements) do
        local menu_button10 = EditDealer:AddButton({
            ...
            select = function(btn)
                ...
                if values == "goto" then
                    TriggerServerEvent('AQCore:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('AQCore:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu7:Close()
                end
            end
...
menu_button4:On('Select', function(item)
    menu7:ClearItems()
    AQCore.Functions.TriggerCallback('test:getdealers', function(dealers)
...
local function OpenPermsMenu(permsply)
    AQCore.Functions.TriggerCallback('aq-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('aq-admin:server:setPermissions', permsply.id, selectedgroup)
			            AQCore.Functions.Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        AQCore.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('aq-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                AQCore.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })
end
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('aq-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                AQCore.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })
end
        ...
        local menu_button10 = Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('aq-admin:server:'..values, player)
...
menu_button2:On('select', function(item)
    menu4:ClearItems()
    AQCore.Functions.TriggerCallback('test:getplayers', function(players)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("aq-weathersync:server:setTime", value, value)
    AQCore.Functions.Notify("Time changed to " .. value .. " hs 00 min")
