	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                ...
                local weaponLabel = AQCore.Shared.Weapons?[killerWeapon]?["label"] or "Unknown"
                local weaponName = AQCore.Shared.Weapons?[killerWeapon]?["name"] or "Unknown_Weapon"
                TriggerServerEvent("aq-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has killed ".. GetPlayerName(player) .." with a **".. weaponLabel .. "** (" .. weaponName .. ")")
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then 
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.6, "RESPAWN OVER: ~r~" .. math.ceil(deathTime) .. "~w~ SECONDES", 255, 255, 255, 255)
                    else
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ HOUD ~r~[E] ("..hold..") SECONDES~w~ OM TE RESPAWNEN ~r~(€"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "JE BLOED DOOD OVER: ~r~" .. math.ceil(LaststandTime) .. "~w~ SECONDES", 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "JE BLOED DOOD OVER: ~r~" .. math.ceil(LaststandTime) .. "~w~ SECONDES, JE KUNT NOG GEHOLPEN WORDEN", 255, 255, 255, 255)
                end
