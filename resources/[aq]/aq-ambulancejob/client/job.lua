...
function TakeOutVehicle(vehicleInfo)
    ...
    AQCore.Functions.SpawnVehicle(vehicleInfo[1], function(veh)
...
RegisterNetEvent('AQCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
...
RegisterNetEvent('AQCore:Client:SetDuty', function(duty)
    onDuty = duty
    TriggerServerEvent("hospital:server:SetDoctor")
end)
...
RegisterNetEvent('hospital:client:AiCall', function()
    ...
    local gender = AQCore.Functions.GetPlayerData().gender
...
RegisterNetEvent('hospital:client:CheckStatus', function()
    ...
    if player ~= -1 and distance < 5.0 then
        ...
        AQCore.Functions.TriggerCallback('hospital:GetPlayerStatus', function(result)
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result["BLEED"] > 0 then
                        ...
                        AQCore.Functions.Notify('Player Is Healthy', 'success')
                    end
                end
...
RegisterNetEvent('hospital:client:RevivePlayer', function()
    AQCore.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                AQCore.Functions.Progressbar("hospital_revive", "Persoon helpen..", 5000, false, true, {
                    ...
                    AQCore.Functions.Notify("Je hebt de peroson geholpen")
            ...
            AQCore.Functions.Notify("Je hebt een verbanddoos nodig", "error")
        end
    end, 'firstaid')
end)

RegisterNetEvent('hospital:client:TreatWounds', function()
    AQCore.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                AQCore.Functions.Progressbar("hospital_healwounds", "Genes wonden..", 5000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
            ...
            AQCore.Functions.Notify("Je hebt geen verband bij", "error")
        end
    end, 'bandage')
end)
    ...
    while true do
        ...
        if LocalPlayer.state['isLoggedIn'] then
            ...
            if PlayerJob.name =="ambulance" then
                for k, v in pairs(Config.Locations["duty"]) do
                    ...
                    if dist < 5 then
                        ...
                        if dist < 1.5 then
                            if onDuty then
                                DrawText3D(v.x, v.y, v.z, "~r~E~w~ - Ga uitdienst")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Ga indienst")
                            end
                ...
                for k, v in pairs(Config.Locations["armory"]) do
                    ...
                    if dist < 4.5 then
                        if onDuty then
                            if dist < 1.5 then
                                sleep = 0
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Opslag")
                                if IsControlJustReleased(0, 38) then
                    ...
                    if dist < 4.5 then
                        ...
                        if dist < 1.5 then
                            if IsPedInAnyVehicle(ped, false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Zet voertuig weg")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Pak voertuig")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(ped, false) then
                ...
                for k, v in pairs(Config.Locations["helicopter"]) do
                    ...
                    if dist < 7.5 then
                        if onDuty then
                            ...
                            if dist < 1.5 then
                                if IsPedInAnyVehicle(ped, false) then
                                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Zet helicopter weg")
                                else
                                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Pak helicopter")
                                end
                                if IsControlJustReleased(0, 38) then
                                    if IsPedInAnyVehicle(ped, false) then
                                        AQCore.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
            ...
            for k, v in pairs(Config.Locations["main"]) do
                ...
                if dist < 1.5 then
                    sleep = 7
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Pak lift naar het dak")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
            ...
            for k, v in pairs(Config.Locations["roof"]) do
                ...
                if dist < 1.5 then
                    sleep = 7
                    DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Pak de lift naar beneden")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
                        while not IsScreenFadedOut() do
                            Wait(10)
