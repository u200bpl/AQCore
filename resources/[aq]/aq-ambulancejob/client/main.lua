    ...
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            if #injured <= Config.AlertShowInfo then
                ...
                limbDamageMsg = "Je hebt te veel pijn.."
            end
            AQCore.Functions.Notify(limbDamageMsg, "primary", 5000)
        end
    end
end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        AQCore.Functions.Notify("Je "..Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
    end
end
...
local function ResetAll()
    ...
    TriggerServerEvent("AQCore:Server:SetMetaData", "hunger", 100)
    TriggerServerEvent("AQCore:Server:SetMetaData", "thirst", 100)
end
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    AQCore.Functions.Notify("You are completely healthy again!")
end)
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    AQCore.Functions.Notify("Je wonden zijn genezen")
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        Wait(5)
        if isRevive then
            AQCore.Functions.Notify("Je word geholpen..")
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('aq-phone:server:sendNewMail', {
            ...
            message = "Geachte " .. gender .. " " .. charinfo.lastname .. ",<br /><br />Hierbij ontvang je een mail namens je laatste bezoek aan het ziekenhuis.<br />De totale kosten kwam op: <strong>$"..amount.."</strong><br /><br />We wensen je een snelle herstel!",
            button = {}
        })
    end)
end)
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            ...
            DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - Stap uit bed..")
        ...
        if LocalPlayer.state['isLoggedIn'] then
            ...
            if #(pos - Config.Locations["checking"]) < 1.5 then
                ...
                if doctorCount >= Config.MinimalDoctors then
                    DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - Bel doktor")
                else
                    DrawText3D(Config.Locations["checking"].x, Config.Locations["checking"].y, Config.Locations["checking"].z, "~g~E~w~ - In checken")
                end
                if IsControlJustReleased(0, 38) then
                    if doctorCount >= Config.MinimalDoctors then
                        ...
                        AQCore.Functions.Progressbar("hospital_checkin", "In Checken..", 2000, false, true, {
                            ...
                            if bedId then
                                ...
                                AQCore.Functions.Notify("Bedden zijn bezet..", "error")
                            end
                        end, function() -- Cancel
                            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
                            AQCore.Functions.Notify("Inchecken gefaald!", "error")
                        end)
                    end
                end
            end
