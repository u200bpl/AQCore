...
RegisterNetEvent('hospital:client:UseBandage', function()
    ...
    AQCore.Functions.Progressbar("use_bandage", "Using bandage..", 4000, false, true, {
        ...
        TriggerServerEvent("AQCore:Server:RemoveItem", "bandage", 1)
        TriggerEvent("inventory:client:ItemBox", AQCore.Shared.Items["bandage"], "remove")
...
RegisterNetEvent('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    AQCore.Functions.Progressbar("use_bandage", "Taking painkillers", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        TriggerServerEvent("AQCore:Server:RemoveItem", "painkillers", 1)
        TriggerEvent("inventory:client:ItemBox", AQCore.Shared.Items["painkillers"], "remove")
        onPainKillers = true
            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", AQCore.Functions.GetPlayerData().citizenid, AQCore.Functions.GetPlayerData().metadata["bloodtype"], coords)
