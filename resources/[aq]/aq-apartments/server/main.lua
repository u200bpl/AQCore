...
RegisterNetEvent('aq-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
...
RegisterNetEvent('aq-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('AQCore:Notify', src, "You got a apartment ("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('AQCore:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
AQCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
AQCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
AQCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
AQCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
AQCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = exports.oxmysql:fetchSync('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
AQCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = AQCore.Functions.GetPlayer(src)
