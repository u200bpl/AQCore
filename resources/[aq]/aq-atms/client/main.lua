...
RegisterNetEvent('aq-atms:client:updateBankInformation', function(banking)
    SendNUIMessage({
...
RegisterNetEvent('aq-atms:client:loadATM', function(cards)
    if cards ~= nil and cards[1] ~= nil then
        ...
        for k, v in pairs(Config.ATMModels) do
            ...
            if atm then 
                ...
                AQCore.Functions.Progressbar("accessing_atm", "Pinpas insteken..", 1500, false, true, {
        ...
        AQCore.Functions.Notify("Je moet eerst een bank pas halen", "error")
    end
end)
...
RegisterNUICallback("doATMWithdraw", function(data, cb)
    if data ~= nil then
        TriggerServerEvent('aq-atms:server:doAccountWithdraw', data)
    end
end)

RegisterNUICallback("loadBankingAccount", function(data, cb)
    AQCore.Functions.TriggerCallback('aq-atms:server:loadBankAccount', function(banking)
...
RegisterNUICallback("removeCard", function(data, cb)
    AQCore.Functions.TriggerCallback('aq-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            SetNuiFocus(false, false)
            SendNUIMessage({
                status = "closeATM"
            })
            AQCore.Functions.Notify('Kaart is verwijderd.', 'success')
        else
            AQCore.Functions.Notify('Kan kaart niet verwijderen', 'error')
        end
    end, data)
end)