...
Citizen.CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('AQCore:Notify', -1, "Daily Withdraw Limit Reset", "success")
    end
end)
...
RegisterCommand('atm', function(source)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            local xCH = AQCore.Functions.GetPlayerByCitizenId(cardHolder)
            if xCH ~= nil then
    ...
    TriggerClientEvent('aq-atms:client:loadATM', src, cards)
end)

-- Event

RegisterServerEvent('aq-atms:server:doAccountWithdraw')
AddEventHandler('aq-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then
        local src = source
        local xPlayer = AQCore.Functions.GetPlayer(src)
        ...
        if dailyWith < Config.DailyLimit then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('AQCore:Notify', src, "Withdraw $" .. data.amount .. ' from credit card. Daily Withdraws: ' .. dailyWithdraws[cardHolder], "success")
                else
            ...
            TriggerClientEvent('aq-atms:client:updateBankInformation', src, banking)
        else
            TriggerClientEvent('AQCore:Notify', src, "You have reached the daily limit", "error")
        end
    end
end)
...
AQCore.Functions.CreateCallback('aq-debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = AQCore.Functions.GetPlayer(src)
...
AQCore.Functions.CreateCallback('aq-debitcard:server:deleteCard', function(source, cb, data)
...
AQCore.Functions.CreateCallback('aq-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = AQCore.Functions.GetPlayer(src)
