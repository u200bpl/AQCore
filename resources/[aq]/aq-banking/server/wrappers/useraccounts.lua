        ...
        if not self.cardActive then
            ...
            exports.oxmysql:execute('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('aq-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = AQCore.Functions.GetPlayer(self.source)
...
RegisterServerEvent('aq-banking:server:registerCurrentAccount')
AddEventHandler('aq-banking:server:registerCurrentAccount', function(cid)
...
RegisterServerEvent('aq-banking:server:registerSavingsAccount')
AddEventHandler('aq-banking:server:registerSavingsAccount', function(cid)
