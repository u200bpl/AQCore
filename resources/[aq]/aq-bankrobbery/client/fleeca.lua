...
AddEventHandler("AQCore:Client:OnPlayerLoaded", function()
    ...
    AQCore.Functions.TriggerCallback('aq-bankrobbery:server:GetConfig', function(config)
        Config = config
    end)
    onDuty = true
    ResetBankDoors()
...
Citizen.CreateThread(function()
    Wait(500)
    if AQCore.Functions.GetPlayerData() ~= nil then
        PlayerJob = AQCore.Functions.GetPlayerData().job
        onDuty = true
    end
end)
...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = AQCore.Shared.Items["electronickit"]["name"], image = AQCore.Shared.Items["electronickit"]["image"]},
        [2] = {name = AQCore.Shared.Items["trojan_usb"]["name"], image = AQCore.Shared.Items["trojan_usb"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumFleecaPolice then
                                                ...
                                                AQCore.Functions.Notify('Er moet minimaal '..Config.MinimumFleecaPolice..' Politie zijn', "error") 
                                            end
                                        end
                                    end
                                end
...
AddEventHandler('electronickit:UseElectronickit', function()
    ...
    if closestBank ~= nil then
        AQCore.Functions.TriggerCallback('aq-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if closestBank ~= nil then
                    local dist = #(pos - Config.SmallBanks[closestBank]["coords"])
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            if not Config.SmallBanks[closestBank]["isOpened"] then 
                                AQCore.Functions.TriggerCallback('AQCore:HasItem', function(result)
                                    if result then 
                                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                                        AQCore.Functions.Progressbar("hack_gate", "Hackapparaat aansluiten..", math.random(5000, 10000), false, true, {
                                            ...
                                            TriggerServerEvent("AQCore:Server:RemoveItem", "electronickit", 1)
                                            TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items["electronickit"], "remove")
                                            TriggerServerEvent("AQCore:Server:RemoveItem", "trojan_usb", 1)
                                            TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items["trojan_usb"], "remove")
                                            ...
                                            if not copsCalled then
                                                ...
                                                if Config.SmallBanks[closestBank]["alarm"] then
                                                    TriggerServerEvent("aq-bankrobbery:server:callCops", "small", closestBank, streetLabel, pos)
                                                    copsCalled = true
                                                end
                                            end
                                        end, function() -- Cancel
                                        ...
                                        AQCore.Functions.Notify("Je mist een item..", "error")
                                    end
                                end, "trojan_usb")
                            else
                                AQCore.Functions.Notify("Lijkt dat de bank al open is..", "error")
                            end
                        else
                            AQCore.Functions.Notify('Er moet minimaal '..Config.MinimumFleecaPolice..' Politie zijn', "error")
                        end
                ...
                AQCore.Functions.Notify("Het veiligheidsslot is actief, het openen van de deur is momenteel niet mogelijk.", "error", 5500)
            end
        end)
    end
end)

RegisterNetEvent('aq-bankrobbery:client:setBankState')
AddEventHandler('aq-bankrobbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('aq-bankrobbery:client:enableAllBankSecurity')
AddEventHandler('aq-bankrobbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('aq-bankrobbery:client:disableAllBankSecurity')
AddEventHandler('aq-bankrobbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('aq-bankrobbery:client:BankSecurity')
AddEventHandler('aq-bankrobbery:client:BankSecurity', function(key, status)
...
function openLocker(bankId, lockerId)
    ...
    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        AQCore.Functions.TriggerCallback('AQCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                AQCore.Functions.Progressbar("open_locker_drill", "De kluis openbreken..", math.random(18000, 30000), false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                    ...
                    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('aq-bankrobbery:server:recieveItem', 'paleto')
                    AQCore.Functions.Notify("Successful!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                AQCore.Functions.Notify("Het lijkt erop dat het kluisslot te sterk is..", "error")
                TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        AQCore.Functions.TriggerCallback('AQCore:HasItem', function(hasItem)
            if hasItem then
                ...
                AQCore.Functions.Progressbar("open_locker_drill", "De kluis openbreken..", math.random(18000, 30000), false, true, {
                    ...
                    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                    TriggerServerEvent('aq-bankrobbery:server:recieveItem', 'pacific')
                    AQCore.Functions.Notify("Succesvol!", "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
        ...
        IsDrilling = true
        AQCore.Functions.Progressbar("open_locker", "De kluis openbreken..", math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('aq-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('aq-bankrobbery:server:recieveItem', 'small')
            AQCore.Functions.Notify("Succesvol!", "success")
            IsDrilling = false
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
...
RegisterNetEvent('aq-bankrobbery:client:setLockerState')
AddEventHandler('aq-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('aq-bankrobbery:client:ResetFleecaLockers')
AddEventHandler('aq-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('aq-bankrobbery:client:robberyCall')
AddEventHandler('aq-bankrobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then 
        ...
        if type == "small" then
            ...
            TriggerEvent('aq-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                ...
                callSign = AQCore.Functions.GetPlayerData().metadata["callsign"],
            })
...
function OnHackDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('aq-bankrobbery:server:setBankState', closestBank, true)
    else
		TriggerEvent('mhacking:hide')
	end
end
