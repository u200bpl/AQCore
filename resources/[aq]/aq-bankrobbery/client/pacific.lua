...
Citizen.CreateThread(function()
    ...
    local requiredItems = {
        [1] = {name = AQCore.Shared.Items["security_card_02"]["name"], image = AQCore.Shared.Items["security_card_02"]["image"]},
    }
    while true do
                                ...
                                if lockerDist < 0.5 then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        if CurrentCops >= Config.MinimumPacificPolice then
                                            ...
                                            AQCore.Functions.Notify('Er moet minimaal '..Config.MinimumPacificPolice..' Politie zijn', "error")
                                        end
                                    end
                                end
                            end
                ...
                if dist < 1.5 then
                    if CurrentCops >= Config.MinimumPacificPolice then
                        if not Config.BigBanks["pacific"]["isOpened"] then 
                                    ...
                                    AQCore.Functions.Progressbar("hack_gate", "Hackapparaat aansluiten..", math.random(5000, 10000), false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("aq-bankrobbery:server:callCops", "pacific", 0, streetLabel, pos)
                                                copsCalled = true
                                            end
                                        end
                                    end, function() -- Cancel
                            ...
                            AQCore.Functions.Notify("Het lijkt erop dat de bank al open is", "error")
                        end
                    else
                        AQCore.Functions.Notify('Er moet minimaal '..Config.MinimumPacificPolice..' Politie zijn', "error")
                    end
...
RegisterNetEvent('aq-bankrobbery:UseBankcardB')
AddEventHandler('aq-bankrobbery:UseBankcardB', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = #(pos - Config.BigBanks["pacific"]["coords"][1])
        ...
        AQCore.Functions.TriggerCallback('aq-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPacificPolice then
                    if not Config.BigBanks["pacific"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems2, false)
                        AQCore.Functions.Progressbar("security_pass", "Please validate ..", math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('aq-doorlock:server:updateState', 1, false)
                            TriggerServerEvent("AQCore:Server:RemoveItem", "security_card_02", 1)
                            if not copsCalled then
                                local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, pos.x, pos.y, pos.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
                                local street1 = GetStreetNameFromHashKey(s1)
                                local street2 = GetStreetNameFromHashKey(s2)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('aq-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
