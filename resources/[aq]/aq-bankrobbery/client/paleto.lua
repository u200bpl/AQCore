...
Citizen.CreateThread(function()
    Citizen.Wait(2000)
    local requiredItems = {
        [1] = {name = AQCore.Shared.Items["security_card_01"]["name"], image = AQCore.Shared.Items["security_card_01"]["image"]},
    }
                                    ...
                                    if lockerDist < 0.5 then
                                        ...
                                        if IsControlJustPressed(0, 38) then
                                            if CurrentCops >= Config.MinimumPaletoPolice then
                                                ...
                                                AQCore.Functions.Notify('Er moet minimaal '..Config.MinimumPaletoPolice..' Politie zijn', "error")
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('aq-bankrobbery:UseBankcardA')
AddEventHandler('aq-bankrobbery:UseBankcardA', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local dist = #(pos - Config.BigBanks["paleto"]["coords"])
        ...
        AQCore.Functions.TriggerCallback('aq-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                if CurrentCops >= Config.MinimumPaletoPolice then
                    if not Config.BigBanks["paleto"]["isOpened"] then 
                        TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                        AQCore.Functions.Progressbar("security_pass", "Kaart valideren..", math.random(5000, 10000), false, true, {
                            ...
                            TriggerServerEvent('aq-bankrobbery:server:setBankState', "paleto", true)
                            TriggerServerEvent("AQCore:Server:RemoveItem", "security_card_01", 1)
                            TriggerServerEvent('aq-doorlock:server:updateState', 4, false)
                            if not copsCalled then
                                ...
                                if Config.BigBanks["paleto"]["alarm"] then
                                    TriggerServerEvent("aq-bankrobbery:server:callCops", "paleto", 0, streetLabel, pos)
                                    copsCalled = true
                                end
                            end
                        end, function() -- Cancel
...
function OpenPaletoDoor()
    TriggerServerEvent('aq-doorlock:server:updateState', 85, false)
