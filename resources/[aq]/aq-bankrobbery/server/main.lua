...
Citizen.CreateThread(function()
    while true do
        ...
        if blackoutActive then
            TriggerEvent("aq-weathersync:server:toggleBlackout")
            TriggerClientEvent("police:client:EnableAllCameras", -1)
            TriggerClientEvent("aq-bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterServerEvent('aq-bankrobbery:server:setBankState')
AddEventHandler('aq-bankrobbery:server:setBankState', function(bankId, state)
    if bankId == "paleto" then
        if not robberyBusy then
            Config.BigBanks["paleto"]["isOpened"] = state
            TriggerClientEvent('aq-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('aq-scoreboard:server:SetActivityBusy', "paleto", true)
            TriggerEvent('aq-bankrobbery:server:setTimeout')
        end
    elseif bankId == "pacific" then
        if not robberyBusy then
            Config.BigBanks["pacific"]["isOpened"] = state
            TriggerClientEvent('aq-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('aq-scoreboard:server:SetActivityBusy', "pacific", true)
            TriggerEvent('aq-bankrobbery:server:setTimeout')
        end
    else
        if not robberyBusy then
            ...
            TriggerClientEvent('aq-bankrobbery:client:setBankState', -1, bankId, state)
            TriggerEvent('aq-banking:server:SetBankClosed', bankId, true)
            TriggerEvent('aq-scoreboard:server:SetActivityBusy', "bankrobbery", true)
            TriggerEvent('aq-bankrobbery:server:SetSmallbankTimeout', bankId)
        end
    end
    robberyBusy = true
end)

RegisterServerEvent('aq-bankrobbery:server:setLockerState')
AddEventHandler('aq-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
    ...
    TriggerClientEvent('aq-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterServerEvent('aq-bankrobbery:server:recieveItem')
AddEventHandler('aq-bankrobbery:server:recieveItem', function(type)
    local src = source
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                 if Config.RewardTypes[itemType].type == "item" then
                     ...
                     TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items[item.item], "add")
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items['security_card_01'], "add")
            end
        else
            ply.Functions.AddItem('weapon_stungun', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items['weapon_stungun'], "add")
        end
    elseif type == "paleto" then
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items['security_card_02'], "add")
            end
        else
            ply.Functions.AddItem('weapon_vintagepistol', 1)
            TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items['weapon_vintagepistol'], "add")
        end
    elseif type == "pacific" then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            ...
            if chance == odd then
                ...
                TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items['weapon_microsmg'], "add")
            else
                ply.Functions.AddItem('weapon_minismg', 1)
                TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items['weapon_minismg'], "add")
            end
...
AQCore.Functions.CreateCallback('aq-bankrobbery:server:isRobberyActive', function(source, cb)
    cb(robberyBusy)
end)

AQCore.Functions.CreateCallback('aq-bankrobbery:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('aq-bankrobbery:server:setTimeout')
AddEventHandler('aq-bankrobbery:server:setTimeout', function()
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
                TriggerEvent('aq-scoreboard:server:SetActivityBusy', "bankrobbery", false)
                TriggerEvent('aq-scoreboard:server:SetActivityBusy', "pacific", false)
                ...
                TriggerClientEvent('aq-bankrobbery:client:ClearTimeoutDoors', -1)
...
RegisterServerEvent('aq-bankrobbery:server:SetSmallbankTimeout')
AddEventHandler('aq-bankrobbery:server:SetSmallbankTimeout', function(BankId)
    if not robberyBusy then
        if not timeOut then
            timeOut = true
            Citizen.CreateThread(function()
                ...
                timeOut = false
                robberyBusy = false
            	TriggerClientEvent('aq-bankrobbery:client:ResetFleecaLockers', -1, BankId)
            	TriggerEvent('aq-banking:server:SetBankClosed', BankId, false)
            end)
	end
    end
end)

RegisterServerEvent('aq-bankrobbery:server:callCops')
AddEventHandler('aq-bankrobbery:server:callCops', function(type, bank, streetLabel, coords)
    local cameraId = 4
    ...
    TriggerClientEvent("aq-bankrobbery:client:robberyCall", -1, type, bank, streetLabel, coords)
    TriggerClientEvent("aq-phone:client:addPoliceAlert", -1, alertData)
end)

RegisterServerEvent('aq-bankrobbery:server:SetStationStatus')
AddEventHandler('aq-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("aq-bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        TriggerEvent("aq-weathersync:server:toggleBlackout")
        TriggerClientEvent("police:client:DisableAllCameras", -1)
        TriggerClientEvent("aq-bankrobbery:client:disableAllBankSecurity", -1)
        blackoutActive = true
    else
        CheckStationHits()
    end
...
function CheckStationHits()
    ...
    if Config.PowerStations[11].hit and Config.PowerStations[1].hit and Config.PowerStations[2].hit then
        ...
        TriggerClientEvent("aq-bankrobbery:client:BankSecurity", 1, false)
        TriggerClientEvent("police:client:SetCamera", -1, 22, false)
        TriggerClientEvent("aq-bankrobbery:client:BankSecurity", 2, false)
    end
    if Config.PowerStations[8].hit and Config.PowerStations[4].hit and Config.PowerStations[5].hit and Config.PowerStations[6].hit then
        ...
        TriggerClientEvent("aq-bankrobbery:client:BankSecurity", 3, false)
    end
    if Config.PowerStations[12].hit and Config.PowerStations[13].hit then
        TriggerClientEvent("police:client:SetCamera", -1, 24, false)
        TriggerClientEvent("aq-bankrobbery:client:BankSecurity", 4, false)
        TriggerClientEvent("police:client:SetCamera", -1, 25, false)
        TriggerClientEvent("aq-bankrobbery:client:BankSecurity", 5, false)
    end
end
...
AQCore.Functions.CreateUseableItem("thermite", function(source, item)
    local Player = AQCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('lighter') ~= nil then
        TriggerClientEvent("thermite:UseThermite", source)
    else
        TriggerClientEvent('AQCore:Notify', source, "You're missing ignition source ", "error")
    end
end)

AQCore.Functions.CreateUseableItem("security_card_01", function(source, item)
    local Player = AQCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_01') ~= nil then
        TriggerClientEvent("aq-bankrobbery:UseBankcardA", source)
    end
end)

AQCore.Functions.CreateUseableItem("security_card_02", function(source, item)
    local Player = AQCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemByName('security_card_02') ~= nil then
        TriggerClientEvent("aq-bankrobbery:UseBankcardB", source)
    end
end)

AQCore.Functions.CreateUseableItem("electronickit", function(source, item)
    local Player = AQCore.Functions.GetPlayer(source)
