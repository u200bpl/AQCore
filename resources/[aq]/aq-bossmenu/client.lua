...
RegisterNetEvent('aq-bossmenu:client:openMenu')
AddEventHandler('aq-bossmenu:client:openMenu', function()
...
menu_button6:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("aq-bossmenu:server:withdrawMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button7:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("aq-bossmenu:server:depositMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button:On("select", function()
    menu3:ClearItems()
    AQCore.Functions.TriggerCallback('aq-bossmenu:server:GetEmployees', function(cb)
    ...
    for k,v in pairs(AQCore.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
        if v and v ~= PlayerId() then
            local menu_button10 = menu4:AddButton({
                ...
                select = function(btn)
                    local select = btn.Value
                    TriggerServerEvent('aq-bossmenu:server:giveJob', GetPlayerServerId(v))
                end
            })
        end
    end
...
function UpdateSociety()
    AQCore.Functions.TriggerCallback('aq-bossmenu:server:GetAccount', function(cb)
        ...
        local menu_button9 = manageroptions:AddButton({
            ...
            select = function(btn)
                ...
                if values == 'promote' then
                    ...
                    if result ~= nil then
                        TriggerServerEvent('aq-bossmenu:server:updateGrade', employee.source, tonumber(result))
                    end
                else
                    TriggerServerEvent('aq-bossmenu:server:fireEmployee', employee.source)
                end
            end
        })
    end
