                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent('aq-carwash:server:washCar')
                                end
                            else
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], 'Auto wassen is niet mogelijk..')
                            end
                        end
                    end
                end
...
RegisterNetEvent('aq-carwash:client:washCar')
AddEventHandler('aq-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    AQCore.Functions.Progressbar("search_cabin", "Voertuig aan het wassen..", math.random(4000, 8000), false, true, {
        ...
        AQCore.Functions.Notify("Wassen geannuleerd..", "error")
        washingVehicle = false
    end)
end)
