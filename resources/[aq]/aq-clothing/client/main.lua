...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("aq-clothes:loadPlayerSkin")
		...
		local dist = #(pos - Config.Stores[k].coords)
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Kleren shoppen')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Haar knippen')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Plastische chirurgie krijgen')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                ...
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Gezicht", selected = true},
                                    })
                                end
                            end
                        end
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    gender = "male"
                                    if AQCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    AQCore.Functions.TriggerCallback('aq-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('aq-clothing:client:openOutfitMenu')
AddEventHandler('aq-clothing:client:openOutfitMenu', function()
    AQCore.Functions.TriggerCallback('aq-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('aq-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('aq-clothing:client:openMenu')
AddEventHandler('aq-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "character", label = "Karakter", selected = true},
        {menu = "clothing", label = "Gezicht", selected = false},
        {menu = "accessoires", label = "Accessoires", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    AQCore.Functions.Notify("Je kunt je enkelband niet afdoen..", "error")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('aq-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('aq-clothing:client:reloadOutfits')
AddEventHandler('aq-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('aq-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    AQCore.Functions.Notify("Je hebt je "..data.outfitName.." verwijderd!")
end)
...
function SaveSkin()
	...
	TriggerServerEvent("aq-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('aq-clothes:client:CreateFirstCharacter')
AddEventHandler('aq-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("aq-clothes:loadSkin")
AddEventHandler("aq-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('aq-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('aq-clothing:client:loadPlayerClothing')
AddEventHandler('aq-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('aq-clothing:client:loadOutfit')
AddEventHandler('aq-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if AQCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        AQCore.Functions.Notify("Je koos voor "..oData.outfitName.."! Druk op Bevestigen om outfit te bevestigen.")
    end
end)
...
RegisterNetEvent("aq-clothing:client:adjustfacewear")
AddEventHandler("aq-clothing:client:adjustfacewear",function(type)
    if AQCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
