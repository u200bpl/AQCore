...
AQClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://aq-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://aq-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://aq-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                AQClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    AQClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://aq-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://aq-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://aq-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://aq-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
AQClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                AQClothing.Open(event.data);
                break;
            case "close":
                AQClothing.Close();
                break;
            case "updateMax":
                AQClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                AQClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                AQClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                AQClothing.ResetValues();
                break;
...
AQClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    AQClothing.Close();
    $.post('https://aq-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    AQClothing.Close();
    $.post('https://aq-clothing/resetOutfit');
});

AQClothing.SetCurrentValues = function(clothingValues) {
...
AQClothing.Open = function(data) {
    ...
    AQClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    AQClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://aq-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://aq-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
AQClothing.Close = function() {
    $.post('https://aq-clothing/close');
...
AQClothing.SetMaxValues = function(maxValues) {
...
AQClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://aq-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://aq-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
