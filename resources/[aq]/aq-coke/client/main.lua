...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                DrawText3Ds(-331.77, -2444.70, 7.36, "~g~G~w~ Cocablad verwerken")
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("aq-coke:server:grindleaves")
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance1 < 15 then
            ...
            if distance1 < 2 then
                DrawText3Ds(509.89, 6478.76, 29.77, "~g~G~w~ Start plukken")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance2 < 2 then
                DrawText3Ds(509.47, 6472.89, 29.77, "~g~G~w~ Start plukken")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
            ...
            if distance3 < 2 then
                DrawText3Ds(509.47, 6467.10, 29.74, "~g~G~w~ Start plukken")
                if IsControlJustPressed(0, 47) then
                    PrepareAnim()
                    PickMinigame()
                end
...
Citizen.CreateThread(function()
    while true do
        ...
        if distance < 6 then
            ...
            if distance < 2 then
                DrawText3Ds(-1078.21, -1678.42, 4.57, "~g~G~w~ Process Crack")
                if IsControlJustPressed(0, 47) then
                    TriggerServerEvent("aq-coke:server:processCrack")
                end
            end
...
RegisterNetEvent('aq-coke:client:grindleavesMinigame')
AddEventHandler('aq-coke:client:grindleavesMinigame', function(source)
    PrepareProcessAnim()
    ProcessMinigame(source)
end)

RegisterNetEvent('aq-coke:client:processCrack')
AddEventHandler('aq-coke:client:processCrack', function(source)
    ProcessCrackMinigame(source)
end)

function pickProcess()
    AQCore.Functions.Progressbar("grind_coke", "Aan het plukken..", math.random(80000,120000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("aq-coke:server:getleaf")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
        ClearPedTasks(PlayerPedId())
        AQCore.Functions.Notify("Process geannuleerd", "error")
    end)
end

function cokeProcess()
    AQCore.Functions.Progressbar("grind_coke", "Coke verwerken..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("aq-coke:server:getcoke")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
...
function crackProcess()
    AQCore.Functions.Progressbar("grind_coke", "Crack verwerking..", math.random(10000, 12000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("aq-coke:server:getcrack")
        ClearPedTasks(PlayerPedId())
        cokepicking = false
    end, function() -- Cancel
        openingDoor = false
...
function PickMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            pickProcess()
            AQCore.Functions.Notify("Je hebt een cocablad geplukt", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            AQCore.Functions.Notify("Je hebt het cocablad verprutst!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokepicking = false
...
function ProcessMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            cokeProcess()
            AQCore.Functions.Notify("Je maakt wat coke!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
            ...
            AQCore.Functions.Notify("Je hebt het proces verpest!", "error")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
            cokeprocess = false
...
function ProcessCrackMinigame(source)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            crackProcess()
            AQCore.Functions.Notify("Je maakt wat crack!", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
        else    
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        TriggerServerEvent("aq-coke:server:cokesell")
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        AQCore.Functions.Notify('Het lijkt erop dat er niemand thuis is..', 'error', 3500)
    end
end
