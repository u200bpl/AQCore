...
AQShared.RandomStr = function(length)
	if length > 0 then
		return AQShared.RandomStr(length-1) .. StringCharset[math.random(1, #StringCharset)]
	else
		return ''
	end
end

AQShared.RandomInt = function(length)
	if length > 0 then
		return AQShared.RandomInt(length-1) .. NumberCharset[math.random(1, #NumberCharset)]
	else
		return ''
	end
end

AQShared.SplitStr = function(str, delimiter)
	local result = { }
	local from  = 1
