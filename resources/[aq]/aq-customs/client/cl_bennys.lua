...
local function saveVehicle()
	...
	local myCar = AQCore.Functions.GetVehicleProperties(veh)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("aq-customs:attemptPurchase", type, upgradeLevel)
                ...
                if nearDefault then
                    ...
                    if nearDefault then
                        if not isPlyInBennys then
                            Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "~p~E~w~ - Enter Benny's", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                            if IsControlJustReleased(1, 38) then
                                if (v.useJob and isAuthorized((AQCore.Functions.GetPlayerData().job.name), k)) or not v.useJob then
                                    TriggerEvent('event:control:bennys', k)
                                else
                                    AQCore.Functions.Notify("Jij bent niet geautoriseerd", "error")
                                end
                            end
                        else
                            disableControls()
...
--#[Event Handlers]#--
RegisterNetEvent("aq-customs:purchaseSuccessful")
AddEventHandler("aq-customs:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    AQCore.Functions.Notify("Aankoop Succesvol")
end)

RegisterNetEvent("aq-customs:purchaseFailed")
AddEventHandler("aq-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    AQCore.Functions.Notify("Niet genoeg geld", "error")
end)
