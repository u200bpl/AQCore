...
local function lockpickFinish(success)
	if success then
		AQCore.Functions.Notify(Lang:t("success.lockpick_success"), 'success', 2500)
		...
		Wait(1800)
		TriggerServerEvent('aq-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		AQCore.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("AQCore:Server:RemoveItem", "advancedlockpick", 1, false)
				TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("AQCore:Server:RemoveItem", "lockpick", 1, false)
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		AQCore.Functions.TriggerCallback('aq-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
RegisterNetEvent('AQCore:Client:OnPlayerLoaded', function()
	AQCore.Functions.TriggerCallback('aq-doorlock:server:setupDoors', function(result)
...
RegisterNetEvent('aq-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('aq-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	local doorData = {}
	local dialog = exports['aq-input']:ShowInput({
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then AQCore.Functions.Notify(Lang:t("error.door_not_found"), 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then AQCore.Functions.Notify(Lang:t("error.door_registered"), 'error') canContinue = true return end
		...
		TriggerServerEvent('aq-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then AQCore.Functions.Notify(Lang:t("error.door_not_found"), 'error') return end
		if entity[1] == entity[2] then AQCore.Functions.Notify(Lang:t("error.same_entity"), 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('aq-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('aq-doorlock:client:newDoorAdded', function(data, id)
	Config.DoorList[id] = data
	TriggerEvent('aq-doorlock:client:setState', PlayerData.source, id, data.locked, false, true, true)
end)
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('aq-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end)
