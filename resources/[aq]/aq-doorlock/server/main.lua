...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and AQCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
AQCore.Functions.CreateCallback('aq-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

AQCore.Functions.CreateCallback('aq-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('aq-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('aq-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		TriggerClientEvent('aq-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('aq-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not AQCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = AQCore.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('aq-doorlock:client:newDoorAdded', -1, configData, data.doorname)
end)

-- Commands

AQCore.Commands.Add('newdoor', 'Create a door (Admin Only)', {}, false, function(source)
	TriggerClientEvent('aq-doorlock:client:addNewDoor', source)
