...
RegisterNetEvent('aq-drugs:client:cornerselling', function(data)
    AQCore.Functions.TriggerCallback('aq-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    AQCore.Functions.Notify('Je begon drugs te verkopen')
                    startLocation = GetEntityCoords(PlayerPedId())
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    AQCore.Functions.Notify('Je bent gestopt met het verkopen van drugs')
                end
            else
                AQCore.Functions.Notify('Je hebt geen drugs bij je', 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
            AQCore.Functions.Notify("Er is niet genoeg politie aanwezig (".. Config.MinimumDrugSalePolice .." Verplicht)", "error")
        end
    end)
end)
...
RegisterNetEvent('aq-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if #availableDrugs <= 0 then
        AQCore.Functions.Notify('Geen drugs meer om te verkopen', 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
...
local function toFarAway()
    AQCore.Functions.Notify('Te ver verplaatst', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            pedDist = #(coords - pedCoords)
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('aq-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                AQCore.Functions.Notify('Je bent beroofd en verloor '..bagAmount..' zakjes(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' voor €'..randomPrice..'? / ~g~G~w~ Aanbod afwijzen')
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('aq-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, 47) then
                        AQCore.Functions.Notify('Aanbod geannuleerd', 'error')
                        hasTarget = false
        ...
        if stealingPed ~= nil and stealData ~= nil then
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    DrawText3D(pedpos.x, pedpos.y, pedpos.z, "~g~E~w~ Raap op")
                    if IsControlJustReleased(0, 38) then
                        ...
                        Wait(2000)
                        ClearPedTasks(ped)
                        TriggerServerEvent("AQCore:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
