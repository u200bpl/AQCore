            ...
            if distance < 20 then
                ...
                if distance < 1.5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('aq-garbagejob:server:PayShit', Earnings, location)
                        Earnings = 0
                    end
                end
            end
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, 38) then
                                AQCore.Functions.TriggerCallback('aq-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        AQCore.Functions.Notify("Je hebt €250,- borg betaald")
                                    else
                                        AQCore.Functions.Notify("U heeft geen borg betaald op dit voertuig..")
                                    end
                                end)
                            end
                        else
                            ...
                            if IsControlJustReleased(0, 38) then
                                AQCore.Functions.TriggerCallback('aq-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        AQCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            SetGarbageRoute()
                                            AQCore.Functions.Notify("Je hebt €250,- borg betaald")
                                            AQCore.Functions.Notify("Je bent begonnen met werken, locatie gemarkeerd op GPS")
                                        end, coords, true)
                                    else
                                        AQCore.Functions.Notify("Je hebt niet genoeg geld.. Borg kosten zijn €1000,-")
                                    end
                                end)
                            end
                        end
                        ...
                        if DeliveryBlip ~= nil then
                            ...
                            if Distance < 20 or hasGarbageBag then
                                ...
                                if not hasGarbageBag then
                                    if CanTakeBag then
                                        if Distance < 1.5 then
                                            DrawText3D2(DeliveryData.coords, "~g~E~w~ - Pak een vuilniszak")
                                    ...
                                    if DoesEntityExist(GarbageVehicle) then
                                        ...
                                        if TruckDist < 2 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "~g~E~w~ - Gooi vuilniszak weg")
                                            if IsControlJustPressed(0, 51) then
                                                hasGarbageBag = false
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        if chance < 26 then
                                                            TriggerServerEvent('aq-garbagejob:server:nano')
                                                        end
                                                        SetGarbageRoute()
                                                        AQCore.Functions.Notify("Alle vuilniszakken zijn weg, ga door naar de volgende locatie")
                                                    else
                                                        -- Finished work
                                                        AQCore.Functions.Notify("Je bent klaar met werken, ga terug naar het depot.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        AQCore.Functions.Notify("Er zijn nog steeds "..AmountOfBags.." zakken over")
                                                    else
                                                        AQCore.Functions.Notify("Er zijn nog steeds "..AmountOfBags.." zakken daar")
                                                    end
                                                    hasGarbageBag = false
                                                end
                                                DeliverAnim()
                                        ...
                                        elseif TruckDist < 10 then
                                            DrawText3D(Coords.x, Coords.y, Coords.z, "Sta hier..")
                                        end
                                    else
