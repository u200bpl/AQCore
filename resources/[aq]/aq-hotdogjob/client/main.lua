...
function StartWorking()
    AQCore.Functions.TriggerCallback('aq-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = PlayerPedId()
            ...
            UpdateUI()
            CheckLoop()
            AQCore.Functions.Notify('You have $250 deposit paid!..', 'success')
        else
            AQCore.Functions.Notify('You have no money for the deposit..', 'error')
        end
    end)
end
...
RegisterNetEvent('aq-hotdogjob:client:UpdateReputation')
AddEventHandler('aq-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('aq-hotdogjob:client:ToggleSell')
AddEventHandler('aq-hotdogjob:client:ToggleSell', function(data)
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            AQCore.Functions.Notify('You are to far from your Hot Dog Stand..', 'error')
        end
    else
        AQCore.Functions.Notify('You do not have a hotdog stand', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
	    ...
	    SellingData.Hotdog = nil
            AQCore.Functions.Notify('You are too far from your stall ...', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 4 then
            if SellingData.Hotdog ~= nil then
                ...
                AQCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x in front of $'..(HotdogsForSale * SellingPrice)..' / [8] Reject')
                if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) then
                    AQCore.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) sold in front of $'..(HotdogsForSale * SellingPrice), 'success')
                    TriggerServerEvent('aq-hotdogjob:server:Sell', HotdogsForSale, SellingPrice)
                    SellingData.HasTarget = false
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    AQCore.Functions.Notify('Customer Refused!', 'error')
                    SellingData.HasTarget = false
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('aq-keyminigame:show')
    TriggerEvent('aq-keyminigame:start', FinishMinigame)
end
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('aq-hotdogjob:server:UpdateReputation', Quality)
        if Config.MyLevel == 1 then
            AQCore.Functions.Notify('You have a '..Config.Stock[Quality].Label..' Hot Dog Made!')
            ...
            if Luck == LuckyNumber then
                AQCore.Functions.Notify('You have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                AQCore.Functions.Notify('You have a '..Config.Stock[Quality].Label..' Hotdog made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            end
        end
    else
        AQCore.Functions.Notify('You have no ('..Config.Stock[Quality].Label..') more about this in front of council..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        AQCore.Functions.TriggerCallback('aq-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                AQCore.Functions.Notify('You have a $250 deposit returned!', 'success')
            else
                AQCore.Functions.Notify('Something is my friend..', 'error')
            end
        end)
    else
        AQCore.Functions.Notify('Your hot dog stand was nowhere to be seen, You will not receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
RegisterNetEvent('aq-hotdogjob:staff:DeletStand')
AddEventHandler('aq-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            AQCore.Functions.Notify('Hotdogstand mode removed!')
        end
    end
end)
