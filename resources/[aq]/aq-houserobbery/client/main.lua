...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    AQCore.Functions.TriggerCallback('aq-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    requiredItems = {
        [1] = {name = AQCore.Shared.Items["lockpick"]["name"], image = AQCore.Shared.Items["lockpick"]["image"]},
        [2] = {name = AQCore.Shared.Items["screwdriverset"]["name"], image = AQCore.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['aq-interior']:CreateTier1HouseFurnished(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('aq-houserobbery:client:ResetHouseState')
AddEventHandler('aq-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('aq-houserobbery:client:enterHouse')
AddEventHandler('aq-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        PoliceCall()
                        TriggerEvent('aq-lockpick:client:openLockpick', lockpickFinish)
                        if math.random(1, 100) <= 85 and not IsWearingHandshoes() then
                            local pos = GetEntityCoords(PlayerPedId())
                            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
                        end
                    else
                        AQCore.Functions.Notify('De deur is al open..', 'error', 3500)
                    end
                else
                    AQCore.Functions.Notify('Niet genoeg politie..', 'error', 3500)
                end
            end
        else
            AQCore.Functions.TriggerCallback('AQCore:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        AQCore.Functions.Notify('Het lijkt erop dat je iets mist...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function lockpickFinish(success)
    if success then
        TriggerServerEvent('aq-houserobbery:server:enterHouse', closestHouse)
        AQCore.Functions.Notify('Het werkte', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = AQCore.Shared.Items["advancedlockpick"]
            if math.random(1, 100) < 20 then
                TriggerServerEvent("AQCore:Server:RemoveItem", "advancedlockpick", 1)
                TriggerEvent('inventory:client:ItemBox', itemInfo, "remove")
            end
        else
            local itemInfo = AQCore.Shared.Items["lockpick"]
        ...
        AQCore.Functions.Notify('It did not work..', 'error', 2500)
    end
end

RegisterNetEvent('aq-houserobbery:client:setHouseState')
AddEventHandler('aq-houserobbery:client:setHouseState', function(house, state)
...
function searchCabin(cabin)
    ...
    TriggerServerEvent('aq-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            ...
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('aq-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('aq-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('aq-houserobbery:client:setCabinState')
AddEventHandler('aq-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('aq-houserobbery:client:SetBusyState')
AddEventHandler('aq-houserobbery:client:SetBusyState', function(cabin, house, bool)
