AQCore.Functions.CreateCallback('aq-houserobbery:server:GetHouseConfig', function(source, cb)
    cb(Config.Houses)
end)

RegisterServerEvent('aq-houserobbery:server:enterHouse')
AddEventHandler('aq-houserobbery:server:enterHouse', function(house)
    local src = source
    ...
    if not Config.Houses[house]["opened"] then
        ...
        TriggerClientEvent('aq-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('aq-houserobbery:client:enterHouse', src, house)
...
function ResetHouseStateTimer(house)
    ...
    Citizen.SetTimeout(time, function()
        ...
        TriggerClientEvent('aq-houserobbery:client:ResetHouseState', -1, house)
    end)
end

RegisterServerEvent('aq-houserobbery:server:searchCabin')
AddEventHandler('aq-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    if itemFound < 4 then
        ...
        for i = 1, itemCount, 1 do
            ...
            local itemInfo = AQCore.Shared.Items[randomItem]
            if math.random(1, 100) == 69 then
                randomItem = "painkillers"
                itemInfo = AQCore.Shared.Items[randomItem]
        ...
        TriggerClientEvent('AQCore:Notify', src, 'De doos is leeg', 'error', 3500)
    end

    Config.Houses[house]["furniture"][cabin]["searched"] = true
    TriggerClientEvent('aq-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterServerEvent('aq-houserobbery:server:SetBusyState')
AddEventHandler('aq-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('aq-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)
