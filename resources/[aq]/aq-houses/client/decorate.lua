...
local function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('aq-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("aq-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('aq-anticheat:client:ToggleDecorate', false)
end
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		AQCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('aq-houses:client:decorate', function()
	Citizen.Wait(500)
	if inside then
		if hasKey then
			...
			AQCore.Functions.Notify("U moet de sleutels van het huis hebben!", "error")
		end
	else
		AQCore.Functions.Notify("Je bent niet in een huis!", "error")
	end
end)
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    AQCore.Functions.TriggerCallback('aq-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = not cursorEnabled
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				AQCore.Functions.Notify('Je bent buiten bereik', 'error')
			end
		end
	end
end)