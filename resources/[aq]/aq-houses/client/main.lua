...
local function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil and tonumber(dist) < 30 then 
            AQCore.Functions.TriggerCallback('aq-houses:server:ProximityKO', function(key, owned)
                hasKey = key
                isOwned = owned
            end, closesthouse)
        end
    end
    TriggerEvent('aq-garages:client:setHouseGarage', closesthouse, hasKey)
end

local function setHouseLocations()
    if closesthouse ~= nil then
        AQCore.Functions.TriggerCallback('aq-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		AQCore.Functions.TriggerCallback('aq-houses:server:getHouseDecorations', function(result)
...
function removeHouseKey(citizenData)
    TriggerServerEvent('aq-houses:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function HouseKeysMenu()
    ...
    MenuTitle = "Sleutels"
    ClearMenu()
    AQCore.Functions.TriggerCallback('aq-houses:server:getHouseKeyHolders', function(holders)
        ped = PlayerPedId();
        MenuTitle = "Sleutelhouders:"
        ClearMenu()
        if holders == nil or next(holders) == nil then
            AQCore.Functions.Notify("No key holders found..", "error", 3500)
            closeMenuFull()
        else
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['aq-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['aq-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['aq-interior']:CreateMichaelShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['aq-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['aq-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['aq-interior']:CreateFranklinShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['aq-interior']:CreateFranklinAuntShell(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['aq-interior']:CreateK4Lester(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['aq-interior']:CreateK4Ranch(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['aq-interior']:CreateK4Trailer(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['aq-interior']:CreateK4Trevor(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['aq-interior']:CreateK4Michael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['aq-interior']:CreateK4Low(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['aq-interior']:CreateK4Mid(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['aq-interior']:CreateK4High(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['aq-interior']:CreateK4High2(coords)
    else
        AQCore.Functions.Notify('Invalid House Tier', 'error')
    end
end
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Citizen.Wait(500)
    TriggerServerEvent('aq-houses:server:SetInsideMeta', house, true)
    TriggerEvent('aq-weathersync:client:DisableSync')
    TriggerEvent('aq-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
end
...
local function leaveOwnedHouse(house)
    if not FrontCam then
        ...
        exports['aq-interior']:DespawnInterior(houseObj, function()
            ...
            TriggerEvent('aq-weed:client:leaveHouse')
            TriggerServerEvent('aq-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
        end)
    end
end
...
RegisterNetEvent('aq-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('aq-houses:client:sellHouse', function()
    if closesthouse and hasKey then
        TriggerServerEvent('aq-houses:server:viewHouse', closesthouse)
    end
end)

RegisterNetEvent('aq-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('aq-houses:client:RequestRing', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)

    if closesthouse ~= nil then
        TriggerServerEvent('aq-houses:server:RingDoor', closesthouse)
    end
end)

AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent('aq-houses:client:setHouses')
    SetClosestHouse()
    TriggerEvent('aq-houses:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerEvent('aq-garages:client:setHouseGarage', closesthouse, hasKey)
    TriggerServerEvent("aq-houses:server:setHouses")
end)
...
RegisterNetEvent('aq-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('aq-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('aq-houses:server:addNewHouse', street, coords, price, tier)
end)

RegisterNetEvent('aq-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('aq-houses:server:addGarage', closesthouse, coords)
    else
        AQCore.Functions.Notify("No house around..", "error")
    end
end)

RegisterNetEvent('aq-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z))
    if dist < 1.5 then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('aq-houses:server:lockHouse', false, closesthouse)
                AQCore.Functions.Notify("House is unlocked!", "success", 2500)
            else
                TriggerServerEvent('aq-houses:server:lockHouse', true, closesthouse)
                AQCore.Functions.Notify("House is locked!", "error", 2500)
            end
        else
            AQCore.Functions.Notify("You don't have the keys of the house...", "error", 3500)
        end
    else
        AQCore.Functions.Notify("There is no door nearby", "error", 3500)
    end
end)

RegisterNetEvent('aq-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        CurrentDoorBell = player
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
        AQCore.Functions.Notify("Someone is ringing the door!")
    end
end)

RegisterNetEvent('aq-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('aq-houses:server:giveHouseKey', playerId, closesthouse)
        else
            AQCore.Functions.Notify("You're not close enough to the door..", "error")
        end
    elseif closesthouse == nil then
        AQCore.Functions.Notify("There is no house near you", "error")
    else
        AQCore.Functions.Notify("No one around!", "error")
    end
end)

RegisterNetEvent('aq-houses:client:removeHouseKey', function(data)
    if closesthouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z))
        if housedist < 5 then
            AQCore.Functions.TriggerCallback('aq-houses:server:getHouseOwner', function(result)
                if AQCore.Functions.GetPlayerData().citizenid == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
                    Menu.hidden = not Menu.hidden
                else
                    AQCore.Functions.Notify("You're not a house owner..", "error")
                end
            end, closesthouse)
        else
            AQCore.Functions.Notify("You're not close enough to the door..", "error")
...
RegisterNetEvent('aq-houses:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('aq-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('aq-houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('aq-houses:client:LastLocationHouse', function(houseId)
    AQCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
RegisterNetEvent('aq-houses:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            AQCore.Functions.TriggerCallback('aq-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('aq-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('aq-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('aq-houses:client:setLocation', function(data)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data.id == 'setstash' then
                TriggerServerEvent('aq-houses:server:setLocation', coords, closesthouse, 1)
            elseif data.id == 'setoutift' then
                TriggerServerEvent('aq-houses:server:setLocation', coords, closesthouse, 2)
            elseif data.id == 'setlogout' then
                TriggerServerEvent('aq-houses:server:setLocation', coords, closesthouse, 3)
            end
        else
            AQCore.Functions.Notify('You do not own this house', 'error')
        end
    else    
        AQCore.Functions.Notify('You are not in a house', 'error')
    end
end)

RegisterNetEvent('aq-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('aq-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local Skillbar = exports['aq-skillbar']:GetSkillbarObject()
    if closesthouse ~= nil then
        AQCore.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = #(pos - vector3(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z))
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('aq-houses:server:lockHouse', false, closesthouse)
                                    AQCore.Functions.Notify('It worked the door is now out.', 'success')
                                    TriggerServerEvent('aq-houses:server:SetHouseRammed', true, closesthouse)
                                ...
                                RamsDone = 0
                                TriggerServerEvent('aq-houses:server:SetRamState', false, closesthouse)
                                AQCore.Functions.Notify('It failed try again.', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('aq-houses:server:SetRamState', true, closesthouse)
                        else
                            AQCore.Functions.Notify('Someone is already working on the door..', 'error')
                        end
                    else
                        AQCore.Functions.Notify('19/5000 This house is already open..', 'error')
                    end
                else
                    AQCore.Functions.Notify('You\'re not near a house..', 'error')
                end
            else
                AQCore.Functions.Notify('There is no police force present..', 'error')
            end
        end)
    else
        AQCore.Functions.Notify('You\'re not near a house..', 'error')
...
RegisterNetEvent('aq-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
end)

RegisterNetEvent('aq-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
end)

RegisterNetEvent('aq-houses:client:ResetHouse', function()
    if closesthouse ~= nil then
        if Config.Houses[closesthouse].IsRammed == nil then
            Config.Houses[closesthouse].IsRammed = false
            TriggerServerEvent('aq-houses:server:SetHouseRammed', false, closesthouse)
            TriggerServerEvent('aq-houses:server:SetRamState', false, closesthouse)
        end
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('aq-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            AQCore.Functions.Notify('You locked the house again..', 'success')
        else
            AQCore.Functions.Notify('This door is not broken open ..', 'error')
        end
    end
end)
...
RegisterNUICallback('HasEnoughMoney', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-houses:server:HasEnoughMoney', function(hasEnough)
    end, data.objectData)
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('aq-houses:server:buyHouse', closesthouse)
end)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if #(pos - vector3(Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)) < 1.5 then
                            ...
                            if IsControlJustPressed(0, 47) then -- G
                                TriggerServerEvent("aq-houses:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
