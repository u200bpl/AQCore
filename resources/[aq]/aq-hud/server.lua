...
AQCore.Commands.Add('cash', 'Check Cash Balance', {}, false, function(source, args)
    local Player = AQCore.Functions.GetPlayer(source)
    local cashamount = Player.PlayerData.money.cash
	TriggerClientEvent('hud:client:ShowAccounts', source, 'cash', cashamount)
end)

AQCore.Commands.Add('bank', 'Check Bank Balance', {}, false, function(source, args)
    local Player = AQCore.Functions.GetPlayer(source)
...
AddEventHandler('hud:server:GainStress', function(amount)
    ...
    if Player ~= nil and Player.PlayerData.job.name ~= 'police' then
        ...
        TriggerClientEvent('AQCore:Notify', src, 'Stress krijgen', 'error', 1500)
	end
end)
...
AddEventHandler('hud:server:RelieveStress', function(amount)
    ...
    if Player ~= nil then
        ...
        Player.Functions.SetMetaData('stress', newStress)
        TriggerClientEvent('hud:client:UpdateStress', src, newStress)
        TriggerClientEvent('AQCore:Notify', src, 'Je bent aan het ontspannen')
	end
end)