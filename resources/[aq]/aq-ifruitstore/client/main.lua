                ...
                if dist < 1.0 then
                    ...
                    if dist < 0.6 then
                        if not requiredItemsShowed2 then
                            requiredItems = {
                                [1] = {name = AQCore.Shared.Items["advancedlockpick"]["name"], image = AQCore.Shared.Items["advancedlockpick"]["image"]},
                            }
                                    ...
                                    if Config.Locations["thermite"].isDone then 
                                        AQCore.Functions.TriggerCallback('AQCore:HasItem', function(hasItem)
                                            if hasItem then
                                                ...
                                                AQCore.Functions.Notify("Je mist een geavanceerde lockpick", "error")
                                            end
                                        end, "advancedlockpick")
                                    else
                                        AQCore.Functions.Notify("Beveiliging is nog steeds actief..", "error")
                                    end
                                else
                                    AQCore.Functions.Notify("Niet genoeg politie", "error")
                                end
                            end
                        end
                    else
...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            AQCore.Functions.Notify("Je hebt je handschoen gescheurd..")
        end
        if math.random(1, 100) <= 10 then
            TriggerServerEvent("AQCore:Server:RemoveItem", "advancedlockpick", 1)
            TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items["advancedlockpick"], "remove")
        end
    end
end
...
function GrabItem(spot)
    ...
    AQCore.Functions.Progressbar("grab_ifruititem", "Item loskoppelen", 10000, false, true, {
        ...
        if not copsCalled then
            ...
            -- if Config.SmallBanks[closestBank]["alarm"] then
                TriggerServerEvent("aq-ifruitstore:server:callCops", streetLabel, pos)
                copsCalled = true
            -- end
        end
        ...
        TriggerServerEvent('aq-ifruitstore:server:setSpotState', "isDone", true, spot)
        TriggerServerEvent('aq-ifruitstore:server:setSpotState', "isBusy", false, spot)
        TriggerServerEvent('aq-ifruitstore:server:itemReward', spot)
        TriggerServerEvent('aq-ifruitstore:server:PoliceAlertMessage', 'Mensen proberen items te stelen in de iFruit Store', pos, true)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('aq-ifruitstore:server:setSpotState', "isBusy", false, spot)
    ...
    if usingSafe then
        if won then
            if not Config.Locations["safe"].isDone then
                ...
                TriggerServerEvent("aq-ifruitstore:server:SafeReward")
                TriggerServerEvent("aq-ifruitstore:server:SetSafeStatus", "isBusy", false)
                TriggerServerEvent("aq-ifruitstore:server:SetSafeStatus", "isDone", false)
                takeAnim()
            end
        end
    end
...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("aq-ifruitstore:server:LoadLocationList")
end)
...
RegisterNetEvent('aq-ifruitstore:client:LoadList')
AddEventHandler('aq-ifruitstore:client:LoadList', function(list)
    ...
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
        if CurrentCops >= 0 then
            ...
            if requiredItemsShowed then
                ...
                requiredItemsShowed = false
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                TriggerServerEvent("aq-ifruitstore:server:SetThermiteStatus", "isBusy", true)
...
RegisterNetEvent('aq-ifruitstore:client:setSpotState')
AddEventHandler('aq-ifruitstore:client:setSpotState', function(stateType, state, spot)
...
RegisterNetEvent('aq-ifruitstore:client:SetSafeStatus')
AddEventHandler('aq-ifruitstore:client:SetSafeStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('aq-ifruitstore:client:SetThermiteStatus')
AddEventHandler('aq-ifruitstore:client:SetThermiteStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('aq-ifruitstore:client:PoliceAlertMessage')
AddEventHandler('aq-ifruitstore:client:PoliceAlertMessage', function(msg, coords, blip)
    if blip then
        ...
        AddTextComponentString("911 - Verdachte situatie in de iFruit Store")
...
RegisterNUICallback('thermitefailed', function()
    ...
    TriggerServerEvent("aq-ifruitstore:server:SetThermiteStatus", "isBusy", false)
    ...
    TriggerServerEvent('aq-ifruitstore:server:PoliceAlertMessage', 'Mensen proberen items te stelen in de iFruit Store', coords, true)
end)

RegisterNUICallback('thermitesuccess', function()
    AQCore.Functions.Notify("The fuses are broken", "success")
    TriggerServerEvent("AQCore:Server:RemoveItem", "thermite", 1)
    local pos = GetEntityCoords(PlayerPedId())
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
        TriggerServerEvent("aq-ifruitstore:server:SetThermiteStatus", "isDone", true)
        TriggerServerEvent("aq-ifruitstore:server:SetThermiteStatus", "isBusy", false)
    end
end)
...
RegisterNetEvent('aq-ifruitstore:client:robberyCall')
AddEventHandler('aq-ifruitstore:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then 
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("10-90: iFruitStore Overval")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
