...
AddEventHandler('inventory:client:CheckOpenState', function(type, id, label)
    local name = AQCore.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
                    ...
                    if vehicle ~= 0 and vehicle ~= nil then
                        ...
                        if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                            if GetVehicleDoorLockStatus(vehicle) < 2 then
                                ...
                                AQCore.Functions.Notify("Voertuig is vergrendeld..", "error")
                                return
                            end
                        else
                            CurrentVehicle = nil
...
for i=1, 6 do 
    ...
    RegisterKeyMapping('slot' .. i, 'gGbruikt het item in slot ' .. i, 'keyboard', i)
end
    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = AQCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
RegisterNUICallback('Notify', function(data, cb)
    AQCore.Functions.Notify(data.message, data.type)
end)
...
AddEventHandler("inventory:client:OpenInventory", function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        ...
        SendNUIMessage({
            ...
            other = other,
            maxweight = AQCore.Config.Player.MaxWeight,
            Ammo = PlayerAmmo,
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = AQCore.Functions.GetPlayerData().items,
        maxweight = AQCore.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    AQCore.Functions.Progressbar("repair_vehicle", "Crafting..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    AQCore.Functions.Progressbar("pickupsnowball", "Sneeuwballen verzamelen..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('AQCore:Server:AddItem', "snowball", 1)
        TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(ped)
...
AddEventHandler("inventory:client:UseWeapon", function(weaponData, shootbool)
    ...
    elseif weaponName == "weapon_stickybomb" then
        ...
        TriggerServerEvent('AQCore:Server:RemoveItem', weaponName, 1)
    ...
    elseif weaponName == "weapon_snowball" then
        ...
        TriggerEvent('weapons:client:SetCurrentWeapon', weaponData, shootbool)
        AQCore.Functions.TriggerCallback("weapon:server:GetWeaponAmmo", function(result)
...
RegisterNUICallback('GetWeaponData', function(data, cb)
    local data = {
        WeaponData = AQCore.Shared.Items[data.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local ped = PlayerPedId()
    local WeaponData = AQCore.Shared.Items[data.WeaponData.name]
    local Attachment = WeaponAttachments[WeaponData.name:upper()][data.AttachmentData.attachment]
    
    AQCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(AQCore.Shared.Items[data.item])
end)
...
RegisterNUICallback("combineItem", function(data)
    ...
    TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    AQCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items[combineData.reward], 'add')
    end, function() -- Cancel
        StopAnimTask(ped, aDict, aLib, 1.0)
        AQCore.Functions.Notify("Gefaald", "error")
    end)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = AQCore.Functions.GetPlayerData().items[1],
        [2] = AQCore.Functions.GetPlayerData().items[2],
        [3] = AQCore.Functions.GetPlayerData().items[3],
        [4] = AQCore.Functions.GetPlayerData().items[4],
        [5] = AQCore.Functions.GetPlayerData().items[5],
        [41] = AQCore.Functions.GetPlayerData().items[41],
    } 
