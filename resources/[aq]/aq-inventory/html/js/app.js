...
$(document).on("click", ".weapon-attachments-back", function(e) {
    e.preventDefault();
    $("#AQCore-inventory").css({ display: "block" });
    $("#AQCore-inventory").animate({
            left: 0 + "vw",
        },
        200
    );
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://aq-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://aq-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#AQCore-inventory").animate({
            ...
            200,
            function() {
                $("#AQCore-inventory").css({ display: "none" });
            }
        );
        $(".weapon-attachments-container").animate({
                left: 0 + "vw",
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://aq-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://aq-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://aq-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://aq-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://aq-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://aq-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://aq-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $("#AQCore-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("https://aq-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#AQCore-inventory").css({ left: "0vw" });
