                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        AQCore.Functions.TriggerCallback('aq-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else
                                                    AQCore.Functions.Notify('je moet een zwaardere wapen hebben..', 'error')
                                                end
                                            else
                                                AQCore.Functions.Notify('Niet genoeg politie ('.. Config.RequiredCops ..')', 'error')
                                            end                
                                        end)
                                    end
                                end
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        AQCore.Functions.Notify("Je hebt een vingerafdruk op het glas achtergelaten", "error")
    end

    smashing = true

    AQCore.Functions.Progressbar("smash_vitrine", "Een display op voorraad", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('aq-jewellery:server:setVitrineState', "isOpened", true, k)
        TriggerServerEvent('aq-jewellery:server:setVitrineState', "isBusy", false, k)
        TriggerServerEvent('aq-jewellery:server:vitrineReward')
        TriggerServerEvent('aq-jewellery:server:setTimeout')
    ...
    TriggerServerEvent('aq-jewellery:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('aq-jewellery:client:setVitrineState')
AddEventHandler('aq-jewellery:client:setVitrineState', function(stateType, state, k)
    Config.Locations[k][stateType] = state
end)

RegisterNetEvent('aq-jewellery:client:setAlertState')
AddEventHandler('aq-jewellery:client:setAlertState', function(bool)
    robberyAlert = bool
end)

RegisterNetEvent('aq-jewellery:client:PoliceAlertMessage')
AddEventHandler('aq-jewellery:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then
        ...
        AddTextComponentString("112 - Verdachte situatie bij het juwelier")
