...
RegisterNetEvent('aq-lapraces:client:StartRaceEditor')
AddEventHandler('aq-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        AQCore.Functions.Notify('je bent al bezig met een race maken.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        AQCore.Functions.Notify('Je hebt nog geen checkpoints geplaatst..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        AQCore.Functions.Notify('Je moet minimaal 10 checkpoints hebben', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 40) or IsDisabledControlJustPressed(0, 40) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        AQCore.Functions.Notify('Je kunt niet hoger gaan dan 15')
                    end
                end
                ...
                if IsControlJustPressed(0, 39) or IsDisabledControlJustPressed(0, 39) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        AQCore.Functions.Notify('Je kunt niet lager gaan dan 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    AQCore.Functions.Notify('Druk nogmaals op [9] om te bevestigen', 'error', 5000)
                else
                    ...
                    AQCore.Functions.Notify('Race-editor geannuleerd!', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('aq-lapraces:server:SaveRace', CreatorData)

    AQCore.Functions.Notify('Race: '..CreatorData.RaceName..' is opgeslagen!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            AQCore.Functions.Notify('Je kunt niet te snel gaan', 'error')
        end
    else
        AQCore.Functions.Notify('Je kunt niet te snel gaan', 'error')
    end
...
RegisterNetEvent('aq-lapraces:client:UpdateRaceRacerData')
AddEventHandler('aq-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('aq-lapraces:client:JoinRace')
AddEventHandler('aq-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('aq-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        AQCore.Functions.Notify('Je bent al in een race..', 'error')
    end
end)

RegisterNetEvent('aq-lapraces:client:LeaveRace')
AddEventHandler('aq-lapraces:client:LeaveRace', function(data)
    AQCore.Functions.Notify('You have completed the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('aq-lapraces:client:RaceCountdown')
AddEventHandler('aq-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('aq-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    AQCore.Functions.Notify('The race will start in 10 seconds', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    AQCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            AQCore.Functions.Notify('GO!', 'success', 1000)
        ...
        AQCore.Functions.Notify('Je bent momenteel niet in een race..', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('aq-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('aq-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('aq-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        AQCore.Functions.Notify('Race eindigd in '..SecondsToClock(CurrentRaceData.TotalTime)..', beste lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        AQCore.Functions.Notify('Race eindigd in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('aq-lapraces:client:PlayerFinishs')
AddEventHandler('aq-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            AQCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is geëindigd met spot: '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('aq-lapraces:client:WaitingDistanceCheck')
AddEventHandler('aq-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            AQCore.Functions.Notify('Ga terug naar de start of je wordt uit de race getrapt: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('aq-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
