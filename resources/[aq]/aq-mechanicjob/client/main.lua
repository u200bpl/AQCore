    ...
    AQCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "mechanic" then
                TriggerServerEvent("AQCore:ToggleDuty")
            end
        end
    end)
    isLoggedIn = true
    AQCore.Functions.TriggerCallback('aq-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    AQCore.Functions.TriggerCallback('aq-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('AQCore:Client:SetDuty')
AddEventHandler('AQCore:Client:SetDuty', function(duty)
    onDuty = duty
end)
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if onDuty then
                    if StashDistance < 20 then
                        ...
                        if StashDistance < 1 then
                            DrawText3Ds(Config.Locations["stash"].x, Config.Locations["stash"].y, Config.Locations["stash"].z, "~g~E~w~ - Open Stash")
                ...
                if onDuty then
                    if VehicleDistance < 20 then
                        ...
                        if VehicleDistance < 1 then
                            ...
                            if InVehicle then
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '~g~E~w~ - Voertuig wegzetten')
                                if IsControlJustPressed(0, 38) then
                                    DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                end
                            else
                                DrawText3Ds(Config.Locations["vehicle"].x, Config.Locations["vehicle"].y, Config.Locations["vehicle"].z, '~g~E~w~ - Voertuig ophalen')
                                if IsControlJustPressed(0, 38) then
                                    if IsControlJustPressed(0, 38) then
                                        VehicleList()
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "~r~E~w~ - Off Duty")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "~g~E~w~ - On Duty")
                        end
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("AQCore:ToggleDuty")
                        end
                    ...
                    for k, v in pairs(Config.Plates) do
                        if v.AttachedVehicle == nil then
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            DrawText3Ds(v.coords.x, v.coords.y, v.coords.z + 0.3, "~g~E~w~ - Plaats het voertuig op het platform")
                                            if IsControlJustPressed(0, 38) then
                                                DoScreenFadeOut(150)
                                                Wait(150)
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                TriggerServerEvent('aq-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            AQCore.Functions.Notify("Je kunt geen fietsen op het platform zetten!", "error")
                                        end
                                    end
                                end
                            end
                            ...
                            if PlateDistance < 3 then
                                inRange = true
                                DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "~g~E~w~ - Open Menu")
                                if IsControlJustPressed(0, 38) then
                                    OpenMenu()
                                    Menu.hidden = not Menu.hidden
                                end
...
function SpawnListVehicle(model)
    ...
    AQCore.Functions.SpawnVehicle(model, function(veh)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..AQCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    AQCore.Functions.TriggerCallback('aq-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            AQCore.Functions.Progressbar("repair_part", "Repareer " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerEvent('aq-vehicletuning:client:RepaireeePart', part)
                TriggerServerEvent('aq-inventory:server:SaveStashItems', "mechanicstash", StashItems)
                SetTimeout(250, function()
                    PartsMenu()
                end)
            end, function()
                AQCore.Functions.Notify("Repareer geannuleerd", "error")
            end)
        else
            AQCore.Functions.Notify('Er zijn niet genoeg materialen in de kluis', 'error')
        end
    end, "mechanicstash")
end
...
RegisterNetEvent('aq-vehicletuning:client:RepaireeePart')
AddEventHandler('aq-vehicletuning:client:RepaireeePart', function(part)
    ...
    AQCore.Functions.Notify("De "..Config.ValuesLabels[part].." is gerepareerd")
end)
...
function UnattachVehicle()
    ...
    TriggerServerEvent('aq-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
end

RegisterNetEvent('aq-vehicletuning:client:SetAttachedVehicle')
AddEventHandler('aq-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        AQCore.Functions.Notify("Status onbekend", "error")
                    end
                else
                    AQCore.Functions.Notify("Geen geldig voertuig", "error")
                end
            else
                AQCore.Functions.Notify("U bent niet dicht genoeg bij het voertuig", "error")
            end
        else
            AQCore.Functions.Notify("Je moet eerst in het voertuig zitten", "error")
        end
    else
        AQCore.Functions.Notify("U moet buiten het voertuig zijn", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            AQCore.Functions.Notify("Je bent niet de bestuurder of op een fiets", "error")
        end
    else
        AQCore.Functions.Notify("Je zit niet in een voertuig", "error")
    end
end)
    ...
    -- if CanReapair() then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
            ...
            if veh ~= nil and veh ~= 0 then
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            AQCore.Functions.Progressbar("repair_advanced", "Repareer voertuig..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("AQCore:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", AQCore.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("AQCore:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", AQCore.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(PlayerPedId())
                                AQCore.Functions.Notify("Proces geannuleerd", "error")
                            end)
                        else
                            AQCore.Functions.Notify("Geen geldig onderdeel", "error")
                        end
                    else
                        AQCore.Functions.Notify("Geen geldig voertuig", "error")
                    end
                ...
                AQCore.Functions.Notify("Je moet eerst in het voertuig zijn", "error")
            end
        else
            AQCore.Functions.Notify("Je zit niet in een voertuig", "error")
        end
...
function CanReapair()
    ...
    for k, v in pairs(Config.Businesses) do
        retval = exports['aq-companies']:IsEmployee(v)
    end
    return retval
end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..AQCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..AQCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..AQCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..AQCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..AQCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..AQCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..AQCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
