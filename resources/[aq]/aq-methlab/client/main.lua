...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    TriggerServerEvent("aq-methlab:server:LoadLocationList")
end)
...
Citizen.CreateThread(function()
    Wait(500)
    AQCore.Functions.TriggerCallback('aq-methlab:server:GetData', function(data)
        ...
        if InsideMethlab then
            if #(pos - vector3(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z)) < 20 then
                ...
                if #(pos - vector3(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z)) < 1 then
                    DrawText3Ds(Config.Locations["exit"].coords.x, Config.Locations["exit"].coords.y, Config.Locations["exit"].coords.z, '~g~E~w~ - Verlaat methlab')
            ...
            if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 20 then
                ...
                if #(pos - vector3(Config.Locations["break"].coords.x, Config.Locations["break"].coords.y, Config.Locations["break"].coords.z)) < 1 then
                    ...
                    if IsControlJustPressed(0, 47) then
                        TriggerServerEvent("aq-methlab:server:breakMeth")
                    end
                end
            end
        end
                ...
                if  not machineStarted then
                    if not loadIngredients then
                        if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                            DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y,  Config.Tasks["Furnace"].coords.z + 0.2, '~g~G~w~ - Laad Ingredienten')
                            if IsControlJustPressed(0, 47) then
                                TriggerServerEvent("aq-methlab:server:CheckIngredients")
                            end
                        end
                    else
                        if not finishedMachine then
                            if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                                DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y,  Config.Tasks["Furnace"].coords.z + 0.2, '~g~G~w~ - Start machine')
                                if IsControlJustPressed(0, 38) then
                                    StartMachine()
                                end
                            end
                            ...
                            if #(pos - vector3(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z)) < 1 then
                                DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y,  Config.Tasks["Furnace"].coords.z + 0.2, '~g~G~w~ - Krijg Meth')
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent("aq-methlab:server:receivemethtray")
                    ...
                    DrawText3Ds(Config.Tasks["Furnace"].coords.x, Config.Tasks["Furnace"].coords.y, Config.Tasks["Furnace"].coords.z - 0.4, 'Klaar over '..machinetimer..'s')
                end
            end
        end
        Citizen.Wait(3)
...
RegisterNetEvent('aq-methlab:client:breakMeth')
AddEventHandler('aq-methlab:client:breakMeth', function()
    PrepareAnim()
    BreakMinigame()
end)

RegisterNetEvent('aq-methlab:client:loadIngredients')
AddEventHandler('aq-methlab:client:loadIngredients', function()
    ProcessMinigame()
end)
...
function breakingMeth(amount)
    AQCore.Functions.Progressbar("break_meth", "Breaking Meth ..", Config.BreakMethTimer, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("aq-methlab:server:getmethtray", amount)
        ClearPedTasks(PlayerPedId())
    end, function() -- Cancel
    end)
end
...
function ProcessMinigame()
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            TriggerServerEvent('aq-methlab:server:loadIngredients')
            AQCore.Functions.Notify("Je hebt de ingrediënten geladen", "success")
            FailedAttemps = 0
            SucceededAttempts = 0
            NeededAttempts = 0
...
RegisterNetEvent('aq-methlab:client:UseLabKey')
AddEventHandler('aq-methlab:client:UseLabKey', function(labkey)
    if ClosestMethlab == Config.CurrentLab then
        ...
        if dist < 1 then
            if labkey == ClosestMethlab then
                ...
                AQCore.Functions.Notify('Dit is niet de juiste sleutel..', 'error')
            end
        end
    end
end)
                            ...
                            if dist < 1 then
                                 if labkey == ClosestMethlab then
                                    EnterMethlab()
                                     ...
                                     AQCore.Functions.Notify('Het lijkt erop dat er een sleutel nodig is...', 'error')
                                 end
                            end
                        end
                     end
