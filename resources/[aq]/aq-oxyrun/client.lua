...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			AQCore.Functions.Notify('De levering was goed gegaan, je GPS wordt bijgewerkt bij de volgende aflevering', 'success')
		else
			AQCore.Functions.Notify('Het afzetten is mislukt', 'error')
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			AQCore.Functions.Notify('Je bent dicht bij het afleverpunt')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			AQCore.Functions.Notify('Het duurde te lang', 'error')
		end
		if dstcheck < 2.0 and pedCreated then

			local crds = GetEntityCoords(deliveryPed)
			DrawText3Ds(crds["x"],crds["y"],crds["z"], "~g~E~w~")  
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			DrawText3Ds(pillWorker['coords']["x"],pillWorker['coords']["y"],pillWorker['coords']["z"], "~g~E~w~ - €6,500 - Bezorgopdracht (contant betalen + Oxy)") 
	...
	while true do
		...
		if OxyRun then
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				AQCore.Functions.Notify("De dealer geeft je geen locaties meer vanwege de staat van zijn auto", "error")
			else
				if tasking then
					Citizen.Wait(30000)
				else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true
	AQCore.Functions.Notify('Er is gezorgd voor een auto. Je GPS wordt binnenkort bijgewerkt met locaties', 'success')
end)
