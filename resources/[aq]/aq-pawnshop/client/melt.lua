...
AddEventHandler("AQCore:Client:OnPlayerLoaded", function()
    AQCore.Functions.TriggerCallback('aq-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("aq-pawnshop:server:getGoldBars")
                        end
                    else
                        DrawText3D(Config.MeltLocation.x, Config.MeltLocation.y, Config.MeltLocation.z, "~g~E~w~ - Smelt gouden voorwerpen")
                        if IsControlJustReleased(0, 38) then 
                            local waitTime = math.random(10000, 15000)
                            ScrapAnim(1000)
                            AQCore.Functions.Progressbar("drop_golden_stuff", "Grab Items", 1000, false, true, {
                                ...
                                if not Config.IsMelting then
                                    ...
                                    TriggerServerEvent("aq-pawnshop:server:meltItems")
                                end
                            end)
                        end
                    end
                elseif Config.IsMelting and Config.MeltTime > 0 then
                    DrawText3D(Config.MeltLocation.x, Config.MeltLocation.y, Config.MeltLocation.z, "Smelten: " .. Config.MeltTime..'s')
                end
			end
		end
		if not inRange then
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            AQCore.Functions.Progressbar("sell_gold", "Goud verkopen", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                TriggerServerEvent('aq-pawnshop:server:sellGold')
                            end, function() -- Cancel
                                openingDoor = false
                                ClearPedTasks(PlayerPedId())
...
function HasPlayerGold()
	local retval = false
	AQCore.Functions.TriggerCallback('aq-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('aq-pawnshop:client:startMelting')
AddEventHandler('aq-pawnshop:client:startMelting', function()
    if not Config.IsMelting then
...
RegisterNetEvent('aq-pawnshop:client:SetTakeState')
AddEventHandler('aq-pawnshop:client:SetTakeState', function(state)
