...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and isLoggedIn then
        local IsHandcuffed = exports['aq-policejob']:IsHandcuffed()
        if not IsHandcuffed and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] then
            OpenPhone()
        else
            AQCore.Functions.Notify("Action not available at the moment..", "error")
        end
    end
end)
...
RegisterNetEvent('aq-phone:client:RaceNotify')
AddEventHandler('aq-phone:client:RaceNotify', function(message)
        SendNUIMessage({
...
RegisterNetEvent('aq-phone:client:AddRecentCall')
AddEventHandler('aq-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('aq-phone:server:SaveMetaData', PhoneData.MetaData)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            AQCore.Functions.TriggerCallback('aq-phone:server:GetPhoneData', function(pData)   
...
function LoadPhone()
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:GetPhoneData', function(pData)
        ...
        PhoneData.PlayerData = AQCore.Functions.GetPlayerData()
...
RegisterNUICallback('HasPhone', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    AQCore.Functions.TriggerCallback('aq-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            AQCore.Functions.TriggerCallback('aq-phone:server:GetGarageVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            AQCore.Functions.Notify("You don't have a phone", "error")
        end
    end)
end
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('aq-phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('aq-phone:client:UpdateMails')
AddEventHandler('aq-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('aq-phone:server:ClearButtonData', data.mailId)
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('aq-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('aq-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('aq-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('aq-phone:client:UpdateMessages')
AddEventHandler('aq-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
	...
	table.insert(PhoneData.Chats, {
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Waarom stuur je berichten naar jezelf??",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
            ...
            Wait(100)
            AQCore.Functions.TriggerCallback('aq-phone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent("aq-phone-new:client:BankNotify")
AddEventHandler("aq-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank", 
...
RegisterNetEvent('aq-phone:client:NewMailNotify')
AddEventHandler('aq-phone:client:NewMailNotify', function(MailData)
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "Mail",
                text = "Nieuwe e-mail van "..MailData.sender,
    ...
    TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('aq-phone:server:AddAdvert', data.message)
end)

RegisterNetEvent('aq-phone:client:UpdateAdverts')
AddEventHandler('aq-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "Advertisement",
                text = "Nieuwe advertensie van "..LastAd,
                icon = "fas fa-ad",
                color = "#ff8f1a",
                timeout = 2500,
            },
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('aq-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:DeclineInvoice', function(CanPay, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('aq-phone:server:BillingEmail', data, false)
end)

RegisterNetEvent('aq-phone:client:BillingEmail')
AddEventHandler('aq-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('aq-phone:server:sendNewMail', {
            sender = 'Factureringsafdeling',
            subject = 'Factuur betaald',
            message = 'Factuur is betaald van '..name..' In het aantal van €'..data.amount,
        })
    else
        TriggerServerEvent('aq-phone:server:sendNewMail', {
            sender = 'Factureringsafdeling',
            subject = 'Factuur geweigerd',
            message = 'Factuur is geweigerd '..name..' In het aantal van €'..data.amount,
        })
    end
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('aq-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('aq-phone:client:UpdateHashtags')
AddEventHandler('aq-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('aq-phone:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('aq-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                end
            end
        end
    end
    ...
    TriggerServerEvent('aq-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNetEvent('aq-phone:client:TransferMoney')
AddEventHandler('aq-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "AQBank", text = "&#36;"..amount.." is toegevoegd aan uw account!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
end)


RegisterNetEvent('aq-phone:client:UpdateTweets')
AddEventHandler('aq-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
...
RegisterNetEvent('aq-phone:client:GetMentioned')
AddEventHandler('aq-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    Config.PhoneApplications["twitter"].Alerts = AppAlerts
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "U bent genoemd in een Tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('aq-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('aq-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        AQCore.Functions.TriggerCallback('aq-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:GetCallState', function(CanCall, IsOnline, contactData)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('aq-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('aq-phone:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('aq-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('aq-phone:server:SetCallState', false)
...
RegisterNetEvent('aq-phone:client:CancelCall')
AddEventHandler('aq-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
    ...
    if not PhoneData.isOpen then
        SendNUIMessage({ 
            ...
            NotifyData = { 
                title = "Telefoon",
                content = "Het gesprek is beëindigd", 
                icon = "fas fa-phone", 
                timeout = 3500, 
                color = "#e84118",
            }, 
...
RegisterNetEvent('aq-phone:client:GetCalled')
AddEventHandler('aq-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('aq-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('aq-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNetEvent('aq-phone:client:AnswerCall')
AddEventHandler('aq-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('FetchVehicleResults', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
            for k, v in pairs(result) do
                AQCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    local vehicle = AQCore.Functions.GetClosestVehicle()
    local plate = GetVehicleNumberPlateText(vehicle)
    local vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()
    AQCore.Functions.TriggerCallback('aq-phone:server:ScanPlate', function(result)
        AQCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
	    if AQCore.Shared.Vehicles[vehname] ~= nil then
                result.label = AQCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNetEvent('aq-phone:client:addPoliceAlert')
AddEventHandler('aq-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = AQCore.Functions.GetPlayerData().job
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    AQCore.Functions.Notify('GPS Locatie gezet: '..data.alert.title)
...
RegisterNetEvent('aq-phone:client:GiveContactDetails')
AddEventHandler('aq-phone:client:GiveContactDetails', function()
    local ped = PlayerPedId()

    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('aq-phone:server:GiveContactDetails', PlayerId)
    else
        AQCore.Functions.Notify("Niemand in de buurt!", "error")
    end
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
        ...
        if v.name == Name and v.number == Number then
            ...
            --if PhoneData.isOpen then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Telefoon",
                        text = "Je hebt een contact verwijderd!", 
    ...
    TriggerServerEvent('aq-phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('aq-phone:client:AddNewSuggestion')
AddEventHandler('aq-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('aq-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNetEvent('aq-phone:client:RemoveBankMoney')
AddEventHandler('aq-phone:client:RemoveBankMoney', function(amount)
    --if PhoneData.isOpen then
      if amount > 0 then 
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "AQBank",
                text = "€"..amount.." is van uw saldo verwijderd!", 
                icon = "fas fa-university", 
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNetEvent('aq-phone:client:AddTransaction')
AddEventHandler('aq-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('aq-phone:server:AddTransaction', Data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('aq-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('aq-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerServerEvent('aq-lapraces:server:StartRace', data.RaceData.RaceId)
end)

RegisterNetEvent('aq-phone:client:UpdateLapraces')
AddEventHandler('aq-phone:client:UpdateLapraces', function()
    SendNUIMessage({
...
RegisterNUICallback('GetRaces', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('aq-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['aq-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['aq-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('aq-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:GetRacingData', function(RaceData)
        local ped = PlayerPedId()
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('aq-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            AQCore.Functions.Notify('Je bent te ver weg van de race. GPS is ingesteld op de race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['aq-lapraces']:IsInEditor())
    else
        cb(exports['aq-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    AQCore.Functions.Notify("GPS is ingesteld op " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('aq-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    AQCore.Functions.TriggerCallback('aq-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    AQCore.Functions.Notify('GPS is ingesteld!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    AQCore.Functions.TriggerCallback('aq-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('aq-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('aq-phone:server:RemoveInstallation', data.app)
end)

RegisterNetEvent('aq-phone:RefreshPhone')
AddEventHandler('aq-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = AQCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['aq-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
