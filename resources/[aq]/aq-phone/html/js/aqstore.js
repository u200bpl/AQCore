...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    AQ.Phone.Data.Applications[AppData.app] = null;

    $.post('https://aq-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('https://aq-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, AQ.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === AQ.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    AQ.Phone.Data.Applications[Installed.data.app] = Installed.data;
