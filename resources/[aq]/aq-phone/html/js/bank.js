...
$(document).on('click', '.bank-app-account', function(e){
    ...
    AQ.Phone.Notifications.Add("fas fa-university", "QBank", "Account number. copied!", "#badc58", 1750);
});
...
AQ.Phone.Functions.DoBankOpen = function() {
    AQ.Phone.Data.PlayerData.money.bank = (AQ.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(AQ.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&#36; "+AQ.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', AQ.Phone.Data.PlayerData.money.bank);
...
$(document).on('click', '.bank-app-account-actions', function(e){
    AQ.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    AQ.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('https://aq-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    AQ.Phone.Notifications.Add("fas fa-university", "QBank", "You have transfered &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    AQ.Phone.Notifications.Add("fas fa-university", "QBank", "You don't have enough balance!", "#badc58", 1500);
                }
                AQ.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        AQ.Phone.Notifications.Add("fas fa-university", "QBank", "Fill out all fields!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://aq-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                AQ.Phone.Notifications.Add("fas fa-university", "QBank", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://aq-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
AQ.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
...
AQ.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = AQ.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    AQ.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    AQ.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        AQ.Phone.Notifications.Add("fas fa-university", "QBank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    AQ.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});