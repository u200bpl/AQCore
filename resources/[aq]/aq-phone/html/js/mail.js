...
$(document).on('click', '.mail', function(e){
    ...
    AQ.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://aq-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://aq-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
AQ.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(AQ.Phone.Data.PlayerData.charinfo.firstname+"."+AQ.Phone.Data.PlayerData.charinfo.lastname+"@AQCore.com");
...
AQ.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('https://aq-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        AQ.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

AQ.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+AQ.Phone.Data.PlayerData.charinfo.firstname+""+AQ.Phone.Data.PlayerData.charinfo.lastname+" | "+AQ.Phone.Data.PlayerData.charinfo.phone);
