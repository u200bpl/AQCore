...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://aq-phone/ClearRecentAlerts');
...
AQ.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    console.log(AQ.Phone.Data.AnonymousCall)

    $.post('https://aq-phone/CallContact', JSON.stringify({
        ContactData: cData,
        Anonymous: AQ.Phone.Data.AnonymousCall,
    }), function(status){
        if (cData.number !== AQ.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (AQ.Phone.Data.AnonymousCall) {
                            AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "You started a anonymous call!");
                        }
                        $(".phone-call-outgoing").css({"display":"block"});
                        $(".phone-call-incoming").css({"display":"none"});
                        $(".phone-call-ongoing").css({"display":"none"});
                        $(".phone-call-outgoing-caller").html(cData.name);
                        AQ.Phone.Functions.HeaderTextColor("white", 400);
                        AQ.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
                        setTimeout(function(){
                            $(".phone-app").css({"display":"none"});
                            AQ.Phone.Animations.TopSlideDown('.phone-application-container', 400, 0);
                            AQ.Phone.Functions.ToggleApp("phone-call", "block");
                        }, 450);
    
                        CallData.name = cData.name;
                        CallData.number = cData.number;
                    
                        AQ.Phone.Data.currentApplication = "phone-call";
                    } else {
                        AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "You're already in a call!");
                    }
                } else {
                    AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is busy!");
                }
            } else {
                AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call yourself!");
        }
    });
});
...
AQ.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== AQ.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            AQ.Phone.Functions.ToggleApp("phone", "none");
            AQ.Phone.Functions.ToggleApp("whatsapp", "block");
            AQ.Phone.Data.currentApplication = "whatsapp";
        
            $.post('https://aq-phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                AQ.Phone.Functions.SetupChatMessages(chat, {
        ...
        AQ.Phone.Notifications.Add("fa fa-phone-alt", "Phone", "You can't whatsapp yourself..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    AQ.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://aq-phone/EditContact', JSON.stringify({
            ...
            AQ.Phone.Functions.LoadContacts(PhoneContacts);
        });
        AQ.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        AQ.Phone.Notifications.Add("fas fa-exclamation-circle", "Edit Contact", "Fill out all fields!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('https://aq-phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    AQ.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://aq-phone/AddNewContact', JSON.stringify({
        ...
        AQ.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://aq-phone/RemoveSuggestion', JSON.stringify({
        ...
        AQ.Phone.Notifications.Add("fas fa-exclamation-circle", "Add Contact", "Fill out all fields!");
    }
});
        ...
        if (cData.number !== AQ.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is in a call!");
                }
            } else {
                AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "This person is not available!");
            }
        } else {
            AQ.Phone.Notifications.Add("fas fa-phone", "Phone", "You can't call your own number!");
        }
    });
}

CancelOutgoingCall = function() {
    if (AQ.Phone.Data.currentApplication == "phone-call") {
        AQ.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
    ...
    if (!Canceled) {
        if (!AQ.Phone.Data.CallActive) {
            ...
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                AQ.Phone.Functions.HeaderTextColor("white", 400);
                $("."+AQ.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
AQ.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    AQ.Phone.Functions.HeaderTextColor("white", 500);
    AQ.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    AQ.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    AQ.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://aq-phone/AnswerCall');
});

AQ.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
AQ.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
