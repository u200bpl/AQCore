...
RegisterServerEvent('aq-phone:server:AddAdvert')
AddEventHandler('aq-phone:server:AddAdvert', function(msg)
    local src = source
    ...
    TriggerClientEvent('aq-phone:client:UpdateAdverts', -1, Adverts,
        "@" .. Player.PlayerData.charinfo.firstname .. "" .. Player.PlayerData.charinfo.lastname)
end)

function GetOnlineStatus(number)
    local Target = AQCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
AQCore.Functions.CreateCallback('aq-phone:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
        ...
        if invoices[1] ~= nil then
            for k, v in pairs(invoices) do
                local Ply = AQCore.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
        ...
        if garageresult[1] ~= nil then
            for k, v in pairs(garageresult) do
                ...
                if (AQCore.Shared.Vehicles[vehicleModel] ~= nil) and (Garages[v.garage] ~= nil) then
                    v.garage = Garages[v.garage].label
                    v.vehicle = AQCore.Shared.Vehicles[vehicleModel].name
                    v.brand = AQCore.Shared.Vehicles[vehicleModel].brand
                end
...
AQCore.Functions.CreateCallback('aq-phone:server:GetCallState', function(source, cb, ContactData)
    local Target = AQCore.Functions.GetPlayerByPhone(ContactData.number)
...
RegisterServerEvent('aq-phone:server:SetCallState')
AddEventHandler('aq-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterServerEvent('aq-phone:server:RemoveMail')
AddEventHandler('aq-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        ...
        TriggerClientEvent('aq-phone:client:UpdateMails', src, mails)
    end)
end)
...
RegisterServerEvent('aq-phone:server:sendNewMail')
AddEventHandler('aq-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aq-phone:client:NewMailNotify', src, mailData)
...
RegisterServerEvent('aq-phone:server:sendNewMailToOffline')
AddEventHandler('aq-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterServerEvent('aq-phone:server:sendNewEventMail')
AddEventHandler('aq-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = AQCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        exports.oxmysql:insert(
            'INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('aq-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterServerEvent('aq-phone:server:ClearButtonData')
AddEventHandler('aq-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
RegisterServerEvent('aq-phone:server:MentionedPlayer')
AddEventHandler('aq-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(AQCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('aq-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage,
...
RegisterServerEvent('aq-phone:server:CallContact')
AddEventHandler('aq-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = AQCore.Functions.GetPlayer(src)
    local Target = AQCore.Functions.GetPlayerByPhone(TargetData.number)

    if Target ~= nil then
        TriggerClientEvent('aq-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId,
            AnonymousCall)
    end
end)

RegisterServerEvent('aq-phone:server:BillingEmail')
AddEventHandler('aq-phone:server:BillingEmail', function(data, paid)
    for k, v in pairs(AQCore.Functions.GetPlayers()) do
        local target = AQCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. AQCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' ..
                                 AQCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('aq-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. AQCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' ..
                                 AQCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('aq-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

AQCore.Functions.CreateCallback('aq-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = AQCore.Functions.GetPlayer(source)
    local SenderPly = AQCore.Functions.GetPlayerByCitizenId(sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        invoiceMailData = {
            ...
            message = string.format('U heeft een commissiecheque ontvangen van $%s wanneer %s %s een rekening betaald van $%s.', commission, Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    elseif not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Billing Department',
            subject = 'Bill Paid',
            message = string.format('%s %s een rekening betaald van $%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    end
    Ply.Functions.RemoveMoney('bank', amount, "paid-invoice")
    TriggerEvent('aq-phone:server:sendNewMailToOffline', sendercitizenid, invoiceMailData)
    TriggerEvent("aq-bossmenu:server:addAccountMoney", society, amount)
...
AQCore.Functions.CreateCallback('aq-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local Invoices = {}
    local Ply = AQCore.Functions.GetPlayer(source)
    exports.oxmysql:execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = exports.oxmysql:fetchSync('SELECT * FROM phone_invoices WHERE citizenid = ?',
...
AQCore.Commands.Add('bill', 'Bill A Player', {{
    name = 'id',
    help = 'Player ID'
}, {
    name = 'amount',
    ...
    local biller = AQCore.Functions.GetPlayer(source)
    local billed = AQCore.Functions.GetPlayer(tonumber(args[1]))
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('aq-phone:RefreshPhone', billed.PlayerData.source)
                    TriggerClientEvent('AQCore:Notify', source, 'Factuur succesvol verzonden', 'success')
                    TriggerClientEvent('AQCore:Notify', billed.PlayerData.source, 'Nieuwe factuur ontvangen')
                else
                    TriggerClientEvent('AQCore:Notify', source, 'Moet een geldig bedrag boven de €0 zijn', 'error')
                end
            else
                TriggerClientEvent('AQCore:Notify', source, 'Je kunt jezelf niet factureren', 'error')
            end
        else
            TriggerClientEvent('AQCore:Notify', source, 'Speler niet online', 'error')
        end
    else
        TriggerClientEvent('AQCore:Notify', source, 'Geen toegang', 'error')
    end
end)

RegisterServerEvent('aq-phone:server:UpdateHashtags')
AddEventHandler('aq-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('aq-phone:client:UpdateHashtags', -1, Handle, messageData)
end)
...
AQCore.Functions.CreateCallback('aq-phone:server:GetContactPictures', function(source, cb, Chats)
    for k, v in pairs(Chats) do
        local Player = AQCore.Functions.GetPlayerByPhone(v.number)
...
AQCore.Functions.CreateCallback('aq-phone:server:GetContactPicture', function(source, cb, Chat)
    local Player = AQCore.Functions.GetPlayerByPhone(Chat.number)
...
AQCore.Functions.CreateCallback('aq-phone:server:GetPicture', function(source, cb, number)
    local Player = AQCore.Functions.GetPlayerByPhone(number)
    local Picture = nil
...
RegisterServerEvent('aq-phone:server:SetPhoneAlerts')
AddEventHandler('aq-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = AQCore.Functions.GetPlayer(src).citizenid
    QBPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterServerEvent('aq-phone:server:UpdateTweets')
AddEventHandler('aq-phone:server:UpdateTweets', function(NewTweets, TweetData)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('aq-phone:client:UpdateTweets', -1, src, Tweets, TwtData)
end)

RegisterServerEvent('aq-phone:server:TransferMoney')
AddEventHandler('aq-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = AQCore.Functions.GetPlayer(src)

    local query = '%' .. iban .. '%'
    local result = exports.oxmysql:fetchSync('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
        local reciever = AQCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('aq-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('AQCore:Notify', src, "Dit rekeningnummer bestaat niet!", "error")
    end
end)

RegisterServerEvent('aq-phone:server:EditContact')
AddEventHandler('aq-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
RegisterServerEvent('aq-phone:server:RemoveContact')
AddEventHandler('aq-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
RegisterServerEvent('aq-phone:server:AddNewContact')
AddEventHandler('aq-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
RegisterServerEvent('aq-phone:server:UpdateMessages')
AddEventHandler('aq-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
    local SenderData = AQCore.Functions.GetPlayer(src)

    local query = '%' .. ChatNumber .. '%'
    local Player = exports.oxmysql:fetchSync('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if Player[1] ~= nil then
        local TargetData = AQCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        ...
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('aq-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages,
...
RegisterServerEvent('aq-phone:server:AddRecentCall')
AddEventHandler('aq-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = AQCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aq-phone:client:AddRecentCall', src, data, label, type)

    local Trgt = AQCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('aq-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterServerEvent('aq-phone:server:CancelCall')
AddEventHandler('aq-phone:server:CancelCall', function(ContactData)
    local Ply = AQCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('aq-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('aq-phone:server:AnswerCall')
AddEventHandler('aq-phone:server:AnswerCall', function(CallData)
    local Ply = AQCore.Functions.GetPlayerByPhone(CallData.TargetData.number)

    if Ply ~= nil then
        TriggerClientEvent('aq-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterServerEvent('aq-phone:server:SaveMetaData')
AddEventHandler('aq-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
AQCore.Functions.CreateCallback('aq-phone:server:FetchResult', function(source, cb, search)
    local src = source
...
AQCore.Functions.CreateCallback('aq-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, v in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = AQCore.Shared.Vehicles[result[k].vehicle]
...
AQCore.Functions.CreateCallback('aq-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then
        ...
        TriggerClientEvent('AQCore:Notify', src, 'Geen voertuig in de buurt', 'error')
        cb(nil)
    end
end)
...
AQCore.Functions.CreateCallback('aq-phone:server:GetGarageVehicles', function(source, cb)
    ...
    if result[1] ~= nil then
        for k, v in pairs(result) do
            local VehicleData = AQCore.Shared.Vehicles[v.vehicle]
...
AQCore.Functions.CreateCallback('aq-phone:server:HasPhone', function(source, cb)
    local Player = AQCore.Functions.GetPlayer(source)
...
AQCore.Functions.CreateCallback('aq-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = AQCore.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('AQCore:Notify', source, "Dit rekeningnummer bestaat niet!", "error")
            cb(false)
        end
    end
end)

RegisterServerEvent('aq-phone:server:GiveContactDetails')
AddEventHandler('aq-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('aq-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterServerEvent('aq-phone:server:AddTransaction')
AddEventHandler('aq-phone:server:AddTransaction', function(data)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
AQCore.Functions.CreateCallback('aq-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(AQCore.Functions.GetPlayers()) do
        local Player = AQCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
RegisterServerEvent('aq-phone:server:InstallApplication')
AddEventHandler('aq-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('aq-phone:RefreshPhone', src)
end)

RegisterServerEvent('aq-phone:server:RemoveInstallation')
AddEventHandler('aq-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
AQCore.Commands.Add("setmetadata", "Set Player Metadata (God Only)", {}, false, function(source, args)
    local Player = AQCore.Functions.GetPlayer(source)
