...
RegisterNetEvent('evidence:client:SetStatus', function(statusId, time)
    if time > 0 and StatusList[statusId] then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            ...
            AQCore.Functions.Notify(''..CurrentStatusList[statusId].text..'', 'error')
        end
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    AQCore.Functions.Progressbar('clear_blooddrops', 'Bloed zuiveren..', 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            AQCore.Functions.Notify('Bloed is schoon :)')
        end
    end, function() -- Cancel
        AQCore.Functions.Notify('Bloed is niet schoon..', 'error')
    end)
end)
...
RegisterNetEvent('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
        type = weapon,
        serie = serie and serie or 'Serienummer niet zichtbaar..',
        coords = {
            x = coords.x,
            y = coords.y,
            z = coords.z - 0.9
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    AQCore.Functions.Progressbar('clear_casings', 'Kogelmouwen verwijderen..', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            AQCore.Functions.Notify('Kogelmouwen verwijderd :)')
        end
    end, function() -- Cancel
        AQCore.Functions.Notify('Kogelmouwen niet verwijderd', 'error')
    end)
end)
    ...
    while true do
        ...
        if CurrentCasing and CurrentCasing ~= 0 then
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, '~g~G~w~ - Kogelmouw ~b~#' .. Casings[CurrentCasing].type)
                if IsControlJustReleased(0, 47) then
                    ...
                    local info = {
                        ...
                        ammolabel = Config.AmmoLabels[AQCore.Shared.Weapons[Casings[CurrentCasing].type]['ammotype']],
...
--                     if AQCore.Shared.Weapons[currentWeapon]['ammotype'] ~= 'AMMO_PISTOL' then
--                         automatic = true
--                     end
