...
local function IsTargetDead(playerId)
    local retval = false
    AQCore.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Wait(100)
    return retval
...
RegisterNetEvent('police:client:SearchPlayer', function()
    local player, distance = AQCore.Functions.GetClosestPlayer()
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            AQCore.Functions.Progressbar("robbing_player", "Robbing person..", math.random(5000, 7000), false, true, {
...
RegisterNetEvent('police:client:JailPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(time) > 0 then
            ...
            AQCore.Functions.Notify("Tijd moet hoger zijn dan 0..", "error")
        end
    else
        AQCore.Functions.Notify("Niemand in de buurt", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                AQCore.Functions.Notify("Je kunt iemand in een voertuig niet in de boeien slaan", "error")
            end
        else
            AQCore.Functions.Notify("Niemand in de buurt", "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            AQCore.Functions.TriggerCallback('AQCore:HasItem', function(result)
                if result then 
                    ...
                    AQCore.Functions.Notify("Je hebt geen handboeien bij je", "error")
                end
            end, "handcuffs")
        else
            AQCore.Functions.Notify("Niemand in de buurt", "error")
...
RegisterNetEvent('police:client:GetKidnappedDragger', function(playerId)
    AQCore.Functions.GetPlayerData(function(PlayerData)
        ...
        TriggerEvent('aq-kidnapping:client:SetKidnapping', isEscorting)
    end)
end)
...
CreateThread(function()
    while true do
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "mp_arrest_paired", "crook_p2_back_right", 3)) and not AQCore.Functions.GetPlayerData().metadata["isdead"] then
