...
function TakeOutVehicle(vehicleInfo)
    ...
    AQCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
	    SetCarItemsInfo()
...
local function IsArmoryWhitelist() -- being removed
    ...
    if AQCore.Functions.GetPlayerData().job.name == 'police' then
        retval = true
    end
    return retval
end
...
function VehicleList()
    MenuTitle = "Vehicles:"
    ClearMenu()
    local authorizedVehicles = Config.AuthorizedVehicles[AQCore.Functions.GetPlayerData().job.grade.level]
...
function ImpoundVehicleList()
    AQCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ped = PlayerPedId();
        MenuTitle = "Impounded Vehicles:"
        ClearMenu()

        if result == nil then
            AQCore.Functions.Notify("Er zijn geen in beslag genomen voertuigen", "error", 5000)
            closeMenuFull()
        else
            for k, v in pairs(result) do
                ...
                Menu.addButton(AQCore.Shared.Vehicles[v.vehicle]["name"], "TakeOutImpound", v, "Impounded", " Engine: " .. enginePercent .. "%", " Body: " .. bodyPercent.. "%", " Fuel: "..currentFuel.. "%")
            end
        end
    ...
    AQCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                AQCore.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for k, v in pairs(result) do
                            AQCore.Functions.Notify(''..v..'')
                        end
                    end
                end, playerId)
            else
        ...
        if LocalPlayer.state['isLoggedIn'] and PlayerJob.name == "police" then
            ...
            for k, v in pairs(Config.Locations["duty"]) do
                if #(pos - v) < 5 then
                    ...
                    if #(pos - v) < 1.5 then
                        if not onDuty then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Ga in dienst")
                        else
                            DrawText3D(v.x, v.y, v.z, "~r~E~w~ - Ga uit dienst")
                        end
                        if IsControlJustReleased(0, 38) then
                            ...
                            TriggerEvent('aq-policealerts:ToggleDuty', onDuty)
                        end
                    end
                end
            end
            ...
            for k, v in pairs(Config.Locations["evidence"]) do
                if #(pos - v) < 2 then
                    sleep = 5
                    if #(pos - v) < 1.0 then
                        DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Bewijsvoorraad")
                        if IsControlJustReleased(0, 38) then
                            local drawer = Input("Naar welke lade wil jij kijken?", "", 2)
                            if drawer then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", " 1 | Lade "..drawer, {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", " 1 | Lade "..drawer)
                            end
                        end
                ...
                if #(pos - v) < 2 then
                    ...
                    if #(pos - v) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            local drawer = Input("Naar welke lade wil jij kijken?", "", 2)
                            if drawer then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", " 2 | Lade "..drawer, {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", " 2 | Lade "..drawer)
                            end
                        end
                    elseif #(pos - v) < 1.5 then
                ...
                if #(pos - v) < 2 then
                    ...
                    if #(pos - v) < 1.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            local drawer = Input("Naar welke lade wil jij kijken?", "", 2)
                            if drawer then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", " 3 | Lade "..drawer, {
                                    maxweight = 4000000,
                                    slots = 500,
                                })
                                TriggerEvent("inventory:client:SetCurrentStash", " 3 | Lade "..drawer)
                            end
                        end
                    elseif #(pos - v) < 1.5 then
                    ...
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Voertuig opslaan")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Voertuigen")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    AQCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                else
                                    MenuGarage()
                                    currentGarage = k
            ...
            for k, v in pairs(Config.Locations["impound"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 7.5 then
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Voertuig in beslag nemen")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Politie in beslag namen")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    AQCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            ...
            for k, v in pairs(Config.Locations["helicopter"]) do
                if #(pos - vector3(v.x, v.y, v.z)) < 7.5 then
                    if onDuty then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Helikopter opslaan")
                            else
                                DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Pak een ​​helikopter")
                            end
                            if IsControlJustReleased(0, 38) then
                                if IsPedInAnyVehicle(PlayerPedId(), false) then
                                    AQCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
                                    ...
                                    AQCore.Functions.SpawnVehicle(Config.Helicopter, function(veh)
            ...
            for k, v in pairs(Config.Locations["armory"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Wapenkamer")
                            if IsControlJustReleased(0, 38) then
            ...
            for k, v in pairs(Config.Locations["stash"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Persoonlijke voorraad")
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..AQCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..AQCore.Functions.GetPlayerData().citizenid)
                            end
                        end
                    end
                end
            ...
            for k, v in pairs(Config.Locations["fingerprint"]) do
                if #(pos - v) < 4.5 then
                    if onDuty then
                        sleep = 5
                        if #(pos - v) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Vingerafdruk scannen")
                            if IsControlJustReleased(0, 38) then
                                local player, distance = GetClosestPlayer()
                        ...
                        elseif #(pos - v) < 2.5 then
                            DrawText3D(v.x, v.y, v.z, "Vingerscan")
                        end  
                    end
                end
            end
