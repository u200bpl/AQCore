...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    local player = AQCore.Functions.GetPlayerData()
    PlayerJob = player.job
    onDuty = player.job.onduty
    isHandcuffed = false
    TriggerServerEvent("AQCore:Server:SetMetaData", "ishandcuffed", false)
    ...
    if player.metadata.tracker then
        ...
        TriggerEvent('aq-clothing:client:loadOutfit', trackerClothingData)
    else
        local trackerClothingData = {
            outfitData = {
                ["accessory"] = {
    ...
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('aq-phone:server:sendNewMail', {
            ...
            message = "Dear " .. gender .. " " .. charinfo.lastname ..
                ",<br /><br />Het Centraal Justitieel Incassobureau (HCJI) de boetes in rekening gebracht die u van de politie heeft gekregen.<br />There is <strong>€" ..
                amount .. "</strong> van uw rekening afgeschreven.<br /><br />Met vriendelijke groeten,<br />Mr. I.K. Graai",
            button = {}
        })
    end)
end)
...
RegisterNetEvent('police:client:SendPoliceEmergencyAlert', function()
    ...
    TriggerServerEvent("police:server:SendPoliceEmergencyAlert", streetLabel, pos, AQCore.Functions.GetPlayerData().metadata["callsign"])
end)
...
RegisterNetEvent('police:client:GunShotAlert', function(streetLabel, isAutomatic, fromVehicle, coords, vehicleInfo)
    if PlayerJob.name == 'police' and onDuty then
        ...
        if fromVehicle then
            blipText = "Schoten afgevuurd vanuit een voertuig"
            MessageDetails = {
                [1] = {
...
RegisterNetEvent('police:client:VehicleCall', function(pos, alertTitle, streetLabel, modelPlate, modelName)
    if PlayerJob.name == 'police' and onDuty then
        ...
        SetBlipAsShortRange(blip, false)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("Alert: Auto inbraak")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
RegisterNetEvent('police:client:PoliceAlertMessage', function(title, caption, coords) -- Use this for robberies, etc by TriggerServerEvent('police:server:PoliceAlertMessage', title, caption)
    if PlayerJob.name == 'police' and onDuty then
        AQCore.Functions.Notify({text = title, caption = caption}, 'police')
        PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
        local transG = 100
...
RegisterNetEvent('police:server:SendEmergencyMessageCheck', function(message, coords)
    ...
    if PlayerData.job.name == "police" and onDuty then
        AQCore.Functions.Notify({text = '112 oproep ontvangen', caption = message}, 'police')
        ...
        SetBlipAsShortRange(blip, false)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("112 oproep")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
    ...
    elseif PlayerData.job.name == "ambulance" then
        AQCore.Functions.Notify({text = '112 oproep ontvangen', caption = message}, 'ambulance')
        TriggerEvent("police:client:EmergencySound")
        local transG = 250
        local blip = AddBlipForCoord(coords.x, coords.y, coords.z)
        SetBlipSprite(blip, 66)
...
RegisterNetEvent('police:client:Send112AMessage', function(message)
    local PlayerData = AQCore.Functions.GetPlayerData()
    if PlayerData.job.name == "police" and onDuty then
        AQCore.Functions.Notify('Anonieme melding: '..message, 'police')
        TriggerEvent("police:client:EmergencySound")
    end
end)
