...
RegisterNetEvent('police:client:spawnCone', function()
    AQCore.Functions.Progressbar("spawn_object", "Object plaatsen..", 2500, false, true, {
...
RegisterNetEvent('police:client:deleteObject', function()
    ...
    if dist < 5.0 then
        AQCore.Functions.Progressbar("remove_object", "Object verwijderen..", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        ...
        AQCore.Functions.Notify('Er zijn geen spijkermatten meer..', 'error')
    end
end)
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "police" and PlayerJob.onduty then
                            DrawText3D(pos.x, pos.y, pos.z, '~g~E~w~ Spike verwijderen')
