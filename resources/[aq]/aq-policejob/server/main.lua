...
AQCore.Commands.Add("spikestrip", "Place Spike Strip (Police Only)", {}, false, function(source)
    local src = source
...
AQCore.Commands.Add("grantlicense", "Grant a license to someone", {{name = "id", help = "ID of a person"}, {name = "license", help = "License Type"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer then
                ...
                TriggerClientEvent('AQCore:Notify', SearchedPlayer.PlayerData.source, "You have been granted a license",
                    "success", 5000)
                TriggerClientEvent('AQCore:Notify', src, "You granted a license", "success", 5000)
            end
        else
            TriggerClientEvent('AQCore:Notify', src, "Invalid license type", "error")
        end
    else
        TriggerClientEvent('AQCore:Notify', src, "You must be a Sergeant to grant licenses!", "error")
    end
end)

AQCore.Commands.Add("revokelicense", "Revoke a license from someone", {{name = "id", help = "ID of a person"}, {name = "license", help = "License Type"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" then
            ...
            if SearchedPlayer then
                ...
                SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
                TriggerClientEvent('AQCore:Notify', SearchedPlayer.PlayerData.source, "You've had a license revoked",
                    "error", 5000)
                TriggerClientEvent('AQCore:Notify', src, "You revoked a license", "success", 5000)
            end
        else
            TriggerClientEvent('AQCore:Notify', src, "Invalid license type", "error")
        end
    else
        TriggerClientEvent('AQCore:Notify', src, "You must be a Sergeant to revoke licenses!", "error")
    end
end)

AQCore.Commands.Add("pobject", "Place/Delete An Object (Police Only)", {{name = "type",help = "Type object you want or 'delete' to delete"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        TriggerClientEvent('AQCore:Notify', src, 'For on-duty police only', 'error')
    end
end)

AQCore.Commands.Add("cuff", "Cuff Player (Police Only)", {}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("palert", "Make a police alert", {{name = "alert", help = "The Police alert"}}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if args[1] then
            local msg = table.concat(args, " ")
            TriggerClientEvent('AQCore:Notify', -1, ''..msg..'', 'police')
            TriggerEvent("aq-log:server:CreateLog", "palert", "Police alert", "blue", "**" .. GetPlayerName(src) .. "** (CitizenID: " .. Player.PlayerData.citizenid .. " | ID: " .. src .. ") **Alert:** " .. msg, false)
            TriggerClientEvent('police:PlaySound', -1)
        else
            TriggerClientEvent('AQCore:Notify', src, 'No report entered', 'error')
        end
    else
        TriggerClientEvent('AQCore:Notify', src, 'For on-duty police only', 'error')
    end
...
AQCore.Commands.Add("escort", "Escort Player", {}, false, function(source, args)
    local src = source
    TriggerClientEvent("police:client:EscortPlayer", src)
end)

AQCore.Commands.Add("callsign", "Give Yourself A Callsign", {{name = "name", help = "Name of your callsign"}}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

AQCore.Commands.Add("clearcasings", "Clear Area of Casings (Police Only)", {}, false, function(source)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("jail", "Jail Player (Police Only)", {{name = "id", help = "Player ID"}, {name = "time", help = "Time they have to be in jail"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        if time > 0 then
            ...
            TriggerClientEvent('AQCore:Notify', src, 'Cannot sentence for 0', 'error')
        end
    else
        TriggerClientEvent('AQCore:Notify', src, 'For on-duty police only', 'error')
    end
...
AQCore.Commands.Add("unjail", "Unjail Player (Police Only)", {{name = "id", help = "Player ID"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local playerId = tonumber(args[1])
...
AQCore.Commands.Add("clearblood", "Clear The Area of Blood (Police Only)", {}, false, function(source)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("seizecash", "Seize Cash (Police Only)", {}, false, function(source)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("sc", "Soft Cuff (Police Only)", {}, false, function(source)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("cam", "View Security Camera (Police Only)", {{name = "camid", help = "Camera ID"}}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("flagplate", "Flag A Plate (Police Only)", {{name = "plate", help = "License"}, {name = "reason", help = "Reason of flagging the vehicle"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local reason = {}
        ...
        TriggerClientEvent('AQCore:Notify', src, "Vehicle (" .. args[1]:upper() .. ") is flagged for: " .. table.concat(reason, " "))
    else
        TriggerClientEvent('AQCore:Notify', src, 'For on-duty police only', 'error')
    end
end)

AQCore.Commands.Add("unflagplate", "Unflag A Plate (Police Only)", {{name = "plate", help = "License plate"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                ...
                TriggerClientEvent('AQCore:Notify', src, "Vehicle (" .. args[1]:upper() .. ") is unflagged")
            else
                TriggerClientEvent('AQCore:Notify', src, 'Vehicle not flagged', 'error')
            end
        else
            TriggerClientEvent('AQCore:Notify', src, 'Vehicle not flagged', 'error')
        end
...
AQCore.Commands.Add("plateinfo", "Run A Plate (Police Only)", {{name = "plate",help = "License plate"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                TriggerClientEvent('AQCore:Notify', src, 'Vehicle ' .. args[1]:upper() .. ' has been flagged for: ' .. Plates[args[1]:upper()].reason)
            else
                TriggerClientEvent('AQCore:Notify', src, 'Vehicle not flagged', 'error')
            end
        else
...
AQCore.Commands.Add("depot", "Impound With Price (Police Only)", {{name = "price", help = "Price for how much the person has to pay (may be empty)"}}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("impound", "Impound A Vehicle (Police Only)", {}, false, function(source)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("paytow", "Pay Tow Driver (Police Only)", {{name = "id",help = "ID of the player"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local playerId = tonumber(args[1])
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('AQCore:Notify', OtherPlayer.PlayerData.source, 'You were paid $500', 'success')
                TriggerClientEvent('AQCore:Notify', src, 'You paid the tow truck driver')
            else
                TriggerClientEvent('AQCore:Notify', src, 'Not a tow truck driver', 'error')
            end
        end
    else
        TriggerClientEvent('AQCore:Notify', src, 'For on-duty police only', 'error')
...
AQCore.Commands.Add("paylawyer", "Pay Lawyer (Police, Judge Only)", {{name = "id",help = "ID of the player"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('AQCore:Notify', OtherPlayer.PlayerData.source, 'You were paid $500', 'success')
                TriggerClientEvent('AQCore:Notify', src, 'You paid a lawyer')
            else
                TriggerClientEvent('AQCore:Notify', src, 'Person is not a lawyer', "error")
            end
        end
    else
        TriggerClientEvent('AQCore:Notify', src, 'For on-duty police only', 'error')
...
AQCore.Commands.Add("112", "Maak politie melding", {{name = "message", help = "Bericht"}}, true, function(source, args)
    local src = source
    ...
    if Player.Functions.GetItemByName("phone") then
        ...
        TriggerEvent("aq-log:server:CreateLog", "112", "112 melding", "blue", "**" .. GetPlayerName(src) .. "** (CitizenID: " .. Player.PlayerData.citizenid .. " | ID: " .. src .. ") **Alert:** " .. message, false)
    else
        TriggerClientEvent('AQCore:Notify', src, 'You dont have a phone', 'error')
    end
end)

AQCore.Commands.Add("911a", "Stuur een anonieme melding naar de hulpdiensten (Stuurt geen locatie)", {{name = "message", help = "Bericht"}}, true, function(source, args)
    local src = source
    local message = table.concat(args, " ")
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName("phone") then
        ...
        TriggerClientEvent('AQCore:Notify', src, 'Je hebt geen telefoon', 'error')
    end
end)

AQCore.Commands.Add("112r", "Stuur een bericht terug naar een melding", {{name = "id", help = "ID of the alert"}, {name = "message",help = "Bericht"}}, true, function(source, args)
    local src = source
    local OtherPlayer = AQCore.Functions.GetPlayer(tonumber(args[1]))
    table.remove(args, 1)
    local message = table.concat(args, " ")
    if OtherPlayer then
        TriggerClientEvent('AQCore:Notify', OtherPlayer.PlayerData.source, ''..message..'')
        TriggerClientEvent("police:client:EmergencySound", OtherPlayer.PlayerData.source)
        TriggerClientEvent("police:client:CallAnim", src)
    end
end)

AQCore.Commands.Add("anklet", "Tracking enkelband bevestigen (Alleen politie)", {}, false, function(source)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("police:client:CheckDistance", src)
    else
        TriggerClientEvent('AQCore:Notify', src, 'Alleen voor dienstdoende politie', 'error')
    end
end)

AQCore.Commands.Add("ankletlocation", "Krijg de locatie van een enkelbandje", {{"cid", "Citizen ID of the person"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if args[1] then
            local citizenid = args[1]
            local Target = AQCore.Functions.GetPlayerByCitizenId(citizenid)
            if Target then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('AQCore:Notify', src, 'Deze persoon heeft geen enkelband om.', 'error')
                end
            end
        end
    else
...
AQCore.Commands.Add("removeanklet", "Tracking enkelbandje verwijderen (Alleen politie)", {{"cid", "Citizen ID of person"}}, true,function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if args[1] then
            ...
            if Target then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('AQCore:Notify', src, 'Deze persoon heeft geen enkelband', 'error')
                end
            end
        end
    else
...
AQCore.Commands.Add("takedrivinglicense", "Rijbewijs in beslag nemen (Alleen politie)", {}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
AQCore.Commands.Add("takedna", "Take a DNA sanple from a person (empty evidence bag needed) (Police Only)", {{"id", "ID of the person"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    local OtherPlayer = AQCore.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", src, AQCore.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('AQCore:Notify', src, "You must have an empty evidence bag with you", "error")
        end
    end
end)
...
AQCore.Functions.CreateUseableItem("handcuffs", function(source, item)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
AQCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName(item) then
...
local function UpdateBlips()
    local dutyPlayers = {}
    local players = AQCore.Functions.GetAQPlayers()
...
AQCore.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = AQCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

AQCore.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = AQCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
AQCore.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = AQCore.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(AQCore.Shared.Weapons[weapon]["name"])
    local retval = false
    if itemInfo then
...
AQCore.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    local players = AQCore.Functions.GetAQPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
...
AQCore.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
AQCore.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
AQCore.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
    local players = AQCore.Functions.GetAQPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
...
AQCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    local players = AQCore.Functions.GetAQPlayers()
    for k, v in pairs(players) do
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('AQCore:Notify', src, "Vehicle unimpounded!", 'success')
end)

RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    local CuffedPlayer = AQCore.Functions.GetPlayer(playerId)
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = AQCore.Functions.GetPlayer(source)
    local EscortPlayer = AQCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
        if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or
            ...
            TriggerClientEvent('AQCore:Notify', src, "Civilian isn't cuffed or dead", 'error')
        end
    end
end)
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer then
            ...
            TriggerEvent('aq-bossmenu:server:addAccountMoney', "police", price)
            TriggerClientEvent('AQCore:Notify', OtherPlayer.PlayerData.source, "You received a fine of $" .. price)
        end
    end
end)
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer then
            ...
            TriggerClientEvent('AQCore:Notify', src, "You sent the person to prison for " .. time .. " months")
        end
    end
end)
...
RegisterNetEvent('police:server:SendEmergencyMessage', function(coords, message)
    local src = source
    local MainPlayer = AQCore.Functions.GetPlayer(src)
    local alertData = {
...
RegisterNetEvent('police:server:SearchPlayer', function(playerId)
    local src = source
    local SearchedPlayer = AQCore.Functions.GetPlayer(playerId)
    if SearchedPlayer then
        TriggerClientEvent('AQCore:Notify', src, 'Found $'..SearchedPlayer.PlayerData.money["cash"]..' on the civilian')
        TriggerClientEvent('AQCore:Notify', SearchedPlayer.PlayerData.source, "You are being searched")
    end
end)
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, AQCore.Shared.Items["moneybag"], "add")
        TriggerClientEvent('AQCore:Notify', SearchedPlayer.PlayerData.source, 'Your cash was confiscated')
    end
end)
...
RegisterNetEvent('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('AQCore:Notify', SearchedPlayer.PlayerData.source, 'Your driving license has been confiscated')
        else
            TriggerClientEvent('AQCore:Notify', src, 'No drivers license', 'error')
        end
    end
end)
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer then
        ...
        TriggerClientEvent('AQCore:Notify', SearchedPlayer.PlayerData.source, "You have been robbed of $" .. money)
        TriggerClientEvent('AQCore:Notify', Player.PlayerData.source, "You have stolen $" .. money)
    end
end)
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('AQCore:Notify', src, "Vehicle taken into depot for $" .. price .. "!")
        else
            exports.oxmysql:execute(
                'UPDATE player_vehicles SET state = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?',
                {2, body, engine, fuel, plate})
            TriggerClientEvent('AQCore:Notify', src, "Vehicle seized")
        end
    end
end)
...
RegisterNetEvent('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = AQCore.Shared.Weapons[weapon]
...
RegisterNetEvent('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('AQCore:Notify', targetId, 'Your anklet is taken off.', 'error', 5000)
        TriggerClientEvent('AQCore:Notify', src, 'You took off an ankle bracelet from ' .. Target.PlayerData.charinfo.firstname .. " " .. Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('AQCore:Notify', targetId, 'You put on an ankle strap.', 'error', 5000)
        TriggerClientEvent('AQCore:Notify', src, 'You put on an ankle strap to ' .. Target.PlayerData.charinfo.firstname .. " " .. Target.PlayerData.charinfo.lastname, 'error', 5000)
...
RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = AQCore.Functions.GetPlayer(source)
    ...
    TriggerClientEvent("aq-phone:client:addPoliceAlert", requestId, alertData)
end)
