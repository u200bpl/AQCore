...
Citizen.CreateThread(function()
    ...
    requiredItems = {
        ...
        [2] = {name = AQCore.Shared.Items["gatecrack"]["name"], image = AQCore.Shared.Items["gatecrack"]["image"]},
    }
    while true do 
        Citizen.Wait(5)
        inRange = false
...
Citizen.CreateThread(function()
	while true do
        ...
        if #(pos - vector3(Config.Locations["middle"].coords.x, Config.Locations["middle"].coords.y, Config.Locations["middle"].coords.z)) > 200 and inJail then
            ...
            AQCore.Functions.Notify("You escaped... Get the hell out of here.!", "error")
		end
	end
end)
...
AddEventHandler('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then 
        AQCore.Functions.TriggerCallback('AQCore:HasItem', function(result)
            if result then 
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                AQCore.Functions.Progressbar("hack_gate", "Electronic kit plug in..", math.random(5000, 10000), false, true, {
                ...
                AQCore.Functions.Notify("You're missing an item..", "error")
            end
        end, "gatecrack")
    end
end)
...
function OnHackDone(success, timeremaining)
    if success then
		...
		TriggerServerEvent('aq-doorlock:server:updateState', Gates[currentGate].gatekey, false)
