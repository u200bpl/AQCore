...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    AQCore.Functions.TriggerCallback('aq-scoreboard:server:GetConfig', function(config)
        Config.IllegalActions = config
    end)
end)

RegisterNetEvent('aq-scoreboard:client:SetActivityBusy')
AddEventHandler('aq-scoreboard:client:SetActivityBusy', function(activity, busy)
    Config.IllegalActions[activity].busy = busy
end)

RegisterCommand('scoreboard', function()
    if not scoreboardOpen then
        AQCore.Functions.TriggerCallback('aq-scoreboard:server:GetPlayersArrays', function(playerList)
            AQCore.Functions.TriggerCallback('aq-scoreboard:server:GetActivity', function(cops, ambulance)
                AQCore.Functions.TriggerCallback("aq-scoreboard:server:GetCurrentPlayers", function(Players)
