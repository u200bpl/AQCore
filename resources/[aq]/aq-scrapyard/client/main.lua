...
AddEventHandler("AQCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("aq-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										AQCore.Functions.Notify("This Vehicle Cannot Be Scrapped.", "error")
									end
								else
									AQCore.Functions.Notify("You Are Not The Driver", "error")
								end
							end
						end
					end
...
RegisterNetEvent('aq-scapyard:client:setNewVehicles')
AddEventHandler('aq-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = AQCore.Shared.Vehicles[v]
		...
		AQCore.Functions.Notify("You Are Not Allowed To Demolish Vehicles Now", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	AQCore.Functions.Progressbar("scrap_vehicle", "Demolish Vehicle", scrapTime, false, true, {
		...
		TriggerServerEvent("aq-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))
