                ...
                if dist < 10 then
                    ...
                    if dist < 1 then
                        ...
                        if IsControlJustPressed(0, 38) then -- E
                            local ShopItems = {}
                            ShopItems.items = {}
                            AQCore.Functions.TriggerCallback('aq-shops:server:getLicenseStatus', function(result)
                                        ...
                                        for i = 1, #products do
                                            if not products[i].requiredJob then
                                                ...
                                                for i2 = 1, #products[i].requiredJob do
                                                    if AQCore.Functions.GetPlayerData().job.name == products[i].requiredJob[i2] and not products[i].requiresLicense then
                                                        table.insert(ShopItems.items, products[i])
                                                    end
                                                end
                                            end
...
function SetupItems(shop)
    local products = Config.Locations[shop].products
    local playerJob = AQCore.Functions.GetPlayerData().job.name
    local items = {}
...
RegisterNetEvent('aq-shops:client:UpdateShop')
AddEventHandler('aq-shops:client:UpdateShop', function(shop, itemData, amount)
    TriggerServerEvent('aq-shops:server:UpdateShopItems', shop, itemData, amount)
end)

RegisterNetEvent('aq-shops:client:SetShopItems')
AddEventHandler('aq-shops:client:SetShopItems', function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent('aq-shops:client:RestockShopItems')
AddEventHandler('aq-shops:client:RestockShopItems', function(shop, amount)
