...
$(document).on('click', '#submit-spawn', function(evt){
    ...
    if (spawnType !== "appartment") {
        $.post('https://aq-spawn/spawnplayer', JSON.stringify({
            spawnloc: location,
            typeLoc: spawnType
        }));
    } else {
        $.post('https://aq-spawn/chooseAppa', JSON.stringify({
            appType: location,
        }));
    }
});
...
function setupLocations(locations, myHouses) {
    ...
    setTimeout(function(){
        $(parent).append('<div class="location" id="location" data-location="current" data-type="current" data-label="Laatste Locatie"><p><span id="current-location">Last Location</span></p></div>');
