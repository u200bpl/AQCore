...
RegisterNetEvent('aq-taxi:client:DoTaxiNpc')
AddEventHandler('aq-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                AQCore.Functions.Notify('The NPC Is Indicated On Your GPS', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    AQCore.Functions.Notify('Bring The NPC To The Specified Location')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                AQCore.Functions.Notify('You Are Already Doing An NPC Mission')
            end
      --  else
         --   AQCore.Functions.Notify('No NPCs Are Available')
       -- end
    else
        AQCore.Functions.Notify('You Are Not In A Taxi')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        TriggerServerEvent('aq-taxi:server:NpcPay', meterData.currentFare)
                        meterActive = false
                        SendNUIMessage({
                            action = "resetMeter"
                        })
                        AQCore.Functions.Notify('Person Was Dropped Off', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterNetEvent('aq-taxi:client:toggleMeter')
AddEventHandler('aq-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then 
            ...
            AQCore.Functions.Notify('This Vehicle Has No Taxi Meter', 'error')
        end
    else
        AQCore.Functions.Notify('You\'re Not In A Vehicle ', 'error')
    end
end)

RegisterNetEvent('aq-taxi:client:enableMeter')
AddEventHandler('aq-taxi:client:enableMeter', function()
    local ped = PlayerPedId()
    ...
    if meterIsOpen then
        ...
        AQCore.Functions.Notify('The Taxi Meter Is Not Active', 'error')
    end
end)
...
RegisterNetEvent('aq-taxi:client:toggleMuis')
AddEventHandler('aq-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        AQCore.Functions.Notify('No Taxi Meter In Sight', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    AQCore.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
