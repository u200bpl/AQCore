...
Citizen.CreateThread(function()
    ...
    if AQCore.Functions.GetPlayerData() ~= nil then
        ...
        AQCore.Functions.TriggerCallback('aq-traphouse:server:GetTraphousesData', function(trappies)
...
RegisterNetEvent('aq-traphouse:client:EnterTraphouse')
AddEventHandler('aq-traphouse:client:EnterTraphouse', function(code)
    if ClosestTraphouse ~= nil then
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        AQCore.Functions.Notify('This Code Is Incorrect', 'error')
    end
end)
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('aq-traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
                ...
                if InteractDistance < 20 then
                    ...
                    if InteractDistance < 1 then
                        if not IsKeyHolder then
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent('aq-traphouse:server:TakeoverHouse', CurrentTraphouse)
                            end
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, 47) then
                                    AQCore.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, 74) then
                                local TraphouseInventory = {}
                            ...
                            if IsControlJustPressed(0, 38) then
                                TriggerServerEvent("aq-traphouse:server:TakeMoney", CurrentTraphouse)
                            end
                        end
                    end
                end
...
function EnterTraphouse(data)
    ...
    data = exports['aq-interior']:CreateTrevorsShell(coords)
    TraphouseObj = data[1]
    POIOffsets = data[2]
...
function LeaveTraphouse(data)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    exports['aq-interior']:DespawnInterior(TraphouseObj, function()
...
RegisterNetEvent('aq-traphouse:client:TakeoverHouse')
AddEventHandler('aq-traphouse:client:TakeoverHouse', function(TraphouseId)
    local ped = PlayerPedId()

    AQCore.Functions.Progressbar("takeover_traphouse", "Taking Over", math.random(1000, 3000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent('aq-traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        AQCore.Functions.Notify("Acquisitions Canceled", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            AQCore.Functions.Notify(CitizenId..' Has Been Added To The Traphouse!')
        else
            AQCore.Functions.Notify(CitizenId..' This Person Already Has Keys')
        end
    else
        AQCore.Functions.Notify('You Can Give Up To 6 People Access To The Trap House!')
    end
    IsKeyHolder = HasKey(CitizenId)
    IsHouseOwner = IsOwner(CitizenId)
end

RegisterNetEvent('aq-traphouse:client:SyncData')
AddEventHandler('aq-traphouse:client:SyncData', function(k, data)
