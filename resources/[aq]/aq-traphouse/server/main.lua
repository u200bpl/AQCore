RegisterServerEvent('aq-traphouse:server:TakeoverHouse')
AddEventHandler('aq-traphouse:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('aq-traphouse:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('aq-traphouse:server:AddHouseKeyHolder')
AddEventHandler('aq-traphouse:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    ...
    if Config.TrapHouses[TraphouseId] ~= nil then
        ...
        if Config.TrapHouses[TraphouseId].keyholders == nil then
            ...
            TriggerClientEvent('aq-traphouse:client:SyncData', -1, TraphouseId, Config.TrapHouses[TraphouseId])
        else
            if #Config.TrapHouses[TraphouseId].keyholders + 1 <= 6 then
                ...
                TriggerClientEvent('AQCore:Notify', src, 'There Are No Slots Left')
            end
        end
    else
        TriggerClientEvent('AQCore:Notify', src, 'Error Has Occurred')
    end
end)
...
AQCore.Commands.Add("entertraphouse", "Enter traphouse", {}, false, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
 
    TriggerClientEvent('aq-traphouse:client:EnterTraphouse', src)
end)

AQCore.Commands.Add("multikeys", "Give Keys To Traphouse", {{name = "id", help = "Player id"}}, true, function(source, args)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('aq-traphouse:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else
                ...
                TriggerClientEvent('AQCore:Notify', src, 'This Person Already Has Keys', 'error')
            end
        else
            TriggerClientEvent('AQCore:Notify', src, 'You Do Not Own A Traphouse Or Are Not The Owner', 'error')
        end
    else
        TriggerClientEvent('AQCore:Notify', src, 'This Person Is Not In The City', 'error')
    end
end)

RegisterServerEvent('aq-traphouse:server:TakeMoney')
AddEventHandler('aq-traphouse:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
    if Config.TrapHouses[TraphouseId].money ~= 0 then
        ...
        TriggerClientEvent('AQCore:Notify', src, 'There issent any money in the cupboard', 'error')
    end
end)
...
RegisterServerEvent('aq-traphouse:server:RobNpc')
AddEventHandler('aq-traphouse:server:RobNpc', function(Traphouse)
    local src = source
    local Player = AQCore.Functions.GetPlayer(src)
...
AQCore.Functions.CreateCallback('aq-traphouse:server:GetTraphousesData', function(source, cb)
