...
AddEventHandler('AttackTransport:akceptujto', function()
	...
	local xPlayer = AQCore.Functions.GetPlayer(_source)
	local accountMoney = 0
	accountMoney = xPlayer.PlayerData.money["bank"]
if ActiveMission == 0 then
	if accountMoney < ActivationCost then
	TriggerClientEvent('AQCore:Notify', _source, "You need $"..ActivationCost.." in the bank to accept the mission")
	else
	...
	if copsOnDuty >= ActivePolice then
		...
		TriggerClientEvent('AQCore:Notify', _source, 'Need at least '..ActivePolice.. ' SASP to activate the mission.')
    end
	end
else
TriggerClientEvent('AQCore:Notify', _source, 'Someone is already carrying out this mission')
end
end)

RegisterServerEvent('aq-armoredtruckheist:server:callCops')
AddEventHandler('aq-armoredtruckheist:server:callCops', function(streetLabel, coords)
    local place = "Armored Truck"
    local msg = "The Alram has been activated from a "..place.. " at " ..streetLabel

    TriggerClientEvent("aq-armoredtruckheist:client:robberyCall", -1, streetLabel, coords)
...
AddEventHandler('AttackTransport:graczZrobilnapad', function(moneyCalc)
	...
	TriggerClientEvent('inventory:client:ItemBox', _source, AQCore.Shared.Items['markedbills'], "add")

	local chance = math.random(1, 100)
	TriggerClientEvent('AQCore:Notify', _source, 'You took '..bags..' bags of cash from the van')

	if chance >= 95 then
	xPlayer.Functions.AddItem('security_card_01', 1)
	TriggerClientEvent('inventory:client:ItemBox', _source, AQCore.Shared.Items['security_card_01'], "add")
	end
