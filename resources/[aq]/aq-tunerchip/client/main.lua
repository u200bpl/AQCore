...
RegisterNUICallback('save', function(data)
    AQCore.Functions.TriggerCallback('aq-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            AQCore.Functions.Notify('TunerChip v1.05: Vehicle Tuned!', 'error')

            TriggerServerEvent('aq-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('aq-tunerchip:client:TuneStatus')
AddEventHandler('aq-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        AQCore.Functions.TriggerCallback('aq-tunerchip:server:GetStatus', function(status)
            if status then
                AQCore.Functions.Notify('This Vehicle Has Been Tuned', 'success')
            else
                AQCore.Functions.Notify('This Vehicle Has Not Been Tuned', 'error')
            end
        end, plate)
    else
        AQCore.Functions.Notify('No Vehicle Nearby', 'error')
    end
end)
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    AQCore.Functions.Notify('TunerChip v1.05: Vehicle has been reset!', 'error')
end)

RegisterNetEvent('aq-tunerchip:client:openChip')
AddEventHandler('aq-tunerchip:client:openChip', function()
    local ped = PlayerPedId()
    local inVehicle = IsPedInAnyVehicle(ped)

    if inVehicle then
        AQCore.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Vehicle Has Been Reset!", 2000, false, true, {
