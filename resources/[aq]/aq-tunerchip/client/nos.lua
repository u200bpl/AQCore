...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    AQCore.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                AQCore.Functions.Progressbar("use_nos", "Connecting NOS...", 1000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    TriggerEvent("inventory:client:ItemBox", AQCore.Shared.Items['nitrous'], "remove")
                    TriggerServerEvent("AQCore:Server:RemoveItem", 'nitrous', 1)
                    local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
                    local Plate = GetVehicleNumberPlateText(CurrentVehicle)
                    TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
                end)
            else
                AQCore.Functions.Notify("You cannot do that from this seat!", "error")
            end
        else
            AQCore.Functions.Notify('You\'re Not In A Car', 'error')
        end
    else
        AQCore.Functions.Notify('You Already Have NOS Active', 'error')
    end
end)
