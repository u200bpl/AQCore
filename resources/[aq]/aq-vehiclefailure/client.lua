...
RegisterNetEvent('aq-vehiclefailure:client:RepairVehicle')
AddEventHandler('aq-vehiclefailure:client:RepairVehicle', function()
...
function CleanVehicle(vehicle)
	...
	AQCore.Functions.Progressbar("cleaning_vehicle", "Cleaning the car...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		AQCore.Functions.Notify("Vehicle cleaned!")
		SetVehicleDirtLevel(vehicle, 0.1)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('aq-vehiclefailure:server:removewashingkit', vehicle)
		TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items["cleaningkit"], "remove")
...
RegisterNetEvent('aq-vehiclefailure:client:SyncWash')
AddEventHandler('aq-vehiclefailure:client:SyncWash', function(veh)
	SetVehicleDirtLevel(veh, 0.1)
	SetVehicleUndriveable(veh, false)
	WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('aq-vehiclefailure:client:CleanVehicle')
AddEventHandler('aq-vehiclefailure:client:CleanVehicle', function()
	local vehicle = AQCore.Functions.GetClosestVehicle()
...
RegisterNetEvent('aq-vehiclefailure:client:RepairVehicleFull')
AddEventHandler('aq-vehiclefailure:client:RepairVehicleFull', function()
	local vehicle = AQCore.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
...
function RepairVehicleFull(vehicle)
	...
	AQCore.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		AQCore.Functions.Notify("Vehicle repaired!")
		...
		TriggerServerEvent('aq-vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		AQCore.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	...
	AQCore.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('aq-vehiclefailure:removeItem', "repairkit")
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		AQCore.Functions.Notify("Failed!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            AQCore.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['aq-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['aq-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	--exports['aq-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['aq-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end
