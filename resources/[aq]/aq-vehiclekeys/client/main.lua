    ...
    while true do
        ...
        if isLoggedIn then
            ...
            if entering ~= 0 then
                ...
                AQCore.Functions.TriggerCallback('vehiclekeys:CheckOwnership', function(result)
                    if not result then -- if not player owned
                        ...
                        if driver ~= 0 and not IsPedAPlayer(driver) then
                            ...
                            if not lockpicked and lockpickedPlate ~= plate then
                                AQCore.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
...
function LockVehicle()
    ...
    local veh = AQCore.Functions.GetClosestVehicle(pos)
            ...
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 2) then
                        ...
                        AQCore.Functions.Notify("Vehicle locked!")
                    else
                        AQCore.Functions.Notify("Something went wrong with the locking system!")
                    end
                else
                    Citizen.Wait(750)
                    ClearPedTasks(ped)
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 1) then
                        ...
                        Wait(200)
                        SetVehicleLights(veh, 0)
                        AQCore.Functions.Notify("Vehicle unlocked!")
                    else
                        AQCore.Functions.Notify("Something went wrong with the locking system!")
                    end
                end
            else
                AQCore.Functions.Notify('You don\'t have the keys of the vehicle..', 'error')
            end
        end, plate)
    end
end
...
function LockpickDoor(isAdvanced)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local vehicle = AQCore.Functions.GetClosestVehicle(pos)
...
function lockpickFinish(success)
    ...
    if success then
        ...
        AQCore.Functions.Notify('Opened Door!', 'success')
        ...
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        AQCore.Functions.Notify('Someone Called The Police!', 'error')
    end
...
function Hotwire()
    if not HasKey then
        ...
        PoliceCall()
        AQCore.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            ...
            if (math.random() <= Config.HotwireChance) then
                ...
                AQCore.Functions.Notify("Hotwire succeeded!")
            else
                SetVehicleEngineOn(veh, false, false, true)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                AQCore.Functions.Notify("Hotwire failed!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random() <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    if AQCore.Shared.Vehicles[modelName] ~= nil then
                        Name = AQCore.Shared.Vehicles[modelName]["brand"] .. ' ' .. AQCore.Shared.Vehicles[modelName]["name"]
                    else
                        Name = "Unknown"
                    end
...
function RobVehicle(target)
    ...
    AQCore.Functions.Progressbar("rob_keys", "Attempting Robbery..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random()
        if chance <= Config.RobberyChance then
            ...
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            TriggerEvent('vehiclekeys:client:SetOwner', plate)
            AQCore.Functions.Notify('You Got The Keys!', 'success')
            Wait(10000)
            IsRobbing = false
        else
            PoliceCall()
            ...
            TaskReactAndFleePed(target, PlayerPedId())
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            AQCore.Functions.Notify('They Called The Cops!', 'error')
            Wait(10000)
            IsRobbing = false
        end
    end)
