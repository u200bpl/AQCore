...
AQCore.Functions.CreateCallback('vehiclekeys:CheckOwnership', function(source, cb, plate)
    local retval = false
...
AQCore.Functions.CreateCallback('vehiclekeys:CheckHasKey', function(source, cb, plate)
...
AddEventHandler('vehiclekeys:server:GiveVehicleKeys', function(plate, target)
    ...
    if CheckOwner(plate, Player.PlayerData.citizenid) then
        if AQCore.Functions.GetPlayer(target) ~= nil then
            TriggerClientEvent('vehiclekeys:client:SetOwner', target, plate)
            TriggerClientEvent('AQCore:Notify', src, "You gave the keys!")
            TriggerClientEvent('AQCore:Notify', target, "You got the keys!")
        else
            TriggerClientEvent('AQCore:Notify', source,  "Player Not Online", "error")
        end
    else
        TriggerClientEvent('AQCore:Notify', source,  "You Dont Own This Vehicle", "error")
    end
end)

AQCore.Commands.Add("engine", "Toggle Engine", {}, false, function(source, args)
	TriggerClientEvent('vehiclekeys:client:ToggleEngine', source)
end)

AQCore.Commands.Add("givecarkeys", "Give Car Keys", {{name = "id", help = "Player id"}}, true, function(source, args)
	local src = source
...
AQCore.Functions.CreateUseableItem("lockpick", function(source, item)
    local Player = AQCore.Functions.GetPlayer(source)
    TriggerClientEvent("lockpicks:UseLockpick", source, false)
end)

AQCore.Functions.CreateUseableItem("advancedlockpick", function(source, item)
    local Player = AQCore.Functions.GetPlayer(source)
