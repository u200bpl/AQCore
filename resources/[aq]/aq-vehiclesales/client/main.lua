            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        AQCore.Functions.TriggerCallback('aq-occasions:server:getVehicles', function(vehicles)
                ...
                if sellBackDist <= 3.5 and IsPedInAnyVehicle(ped) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        AQCore.Functions.TriggerCallback('aq-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                TriggerServerEvent('aq-occasions:server:sellVehicleBack', sellVehData)
                                AQCore.Functions.DeleteVehicle(GetVehiclePedIsIn(ped))
                            else
                                AQCore.Functions.Notify('This is not your vehicle..', 'error', 3500)
                            end
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, AQCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Price: ~g~$'..Config.OccasionSlots[i]["price"])
                                if Config.OccasionSlots[i]["owner"] == AQCore.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    AQCore.Functions.TriggerCallback('aq-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, 161) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("aq-occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            AQCore.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = AQCore.Functions.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    AQCore.Functions.Notify(data.message, 'error')
end)

RegisterNUICallback('buyVehicle', function()
    local vehData = Config.OccasionSlots[currentVehicle]
    TriggerServerEvent('aq-occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('aq-occasions:client:BuyFinished')
AddEventHandler('aq-occasions:client:BuyFinished', function(vehdata)
    local vehmods = json.decode(vehdata.mods)

    DoScreenFadeOut(250)
    Citizen.Wait(500)
    AQCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        SetVehicleNumberPlateText(veh, vehdata.plate)
        SetEntityHeading(veh, Config.BuyVehicle.w)
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        AQCore.Functions.Notify("Vehicle Bought", "success", 2500)
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        AQCore.Functions.SetVehicleProperties(veh, vehmods)
...
RegisterNetEvent('aq-occasions:client:ReturnOwnedVehicle')
AddEventHandler('aq-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    AQCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        AQCore.Functions.Notify("You vehicle is returned")
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        AQCore.Functions.SetVehicleProperties(veh, vehmods)
...
function SellData(data,model)
    AQCore.Functions.TriggerCallback("aq-vehiclesales:server:CheckModelName",function(DataReturning) 
        local vehicleData = {}
        ...
        vehicleData.mods = AQCore.Functions.GetVehicleProperties(vehicleData.ent)
        vehicleData.desc = data.desc
        TriggerServerEvent('aq-occasions:server:sellVehicle', data.price, vehicleData)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    AQCore.Functions.Notify('Your car has been put up for sale! Price - $'..price, 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('aq-occasion:client:refreshVehicles')
AddEventHandler('aq-occasion:client:refreshVehicles', function()
    if inRange then
        AQCore.Functions.TriggerCallback('aq-occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()
