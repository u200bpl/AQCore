            ...
            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    if CanShoot then
                        ...
                        if AQCore.Shared.Weapons[weapon]["name"] == "weapon_snowball" then
                            TriggerServerEvent('AQCore:Server:RemoveItem', "snowball", 1)
                        else
                            if ammo > 0 then
                        ...
                        if weapon ~= -1569615261 then
                            TriggerEvent('inventory:client:CheckWeapon', AQCore.Shared.Weapons[weapon]["name"])
                            AQCore.Functions.Notify("This weapon is broken and can not be used..", "error")
                            MultiplierAmount = 0
                        end
                    end
                end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedShooting(ped) then
            if ammo - 1 < 1 then
                SetAmmoInClip(ped, GetHashKey(AQCore.Shared.Weapons[weapon]["name"]), 1)
            end
        end
...
AddEventHandler('weapon:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData ~= nil then
        if AQCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and AQCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            local total = GetAmmoInPedWeapon(ped, weapon)
            local found, maxAmmo = GetMaxAmmo(ped, weapon)

            if total < maxAmmo then
                AQCore.Functions.Progressbar("taking_bullets", "Kogels laden..", math.random(4000, 6000), false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    if AQCore.Shared.Weapons[weapon] ~= nil then
                        AddAmmoToPed(ped,weapon,amount)
                        TaskReloadWeapon(ped)
                        TriggerServerEvent("weapons:server:AddWeaponAmmo", CurrentWeaponData, total + amount)
                        TriggerServerEvent('AQCore:Server:RemoveItem', itemData.name, 1, itemData.slot)
                        TriggerEvent('inventory:client:ItemBox', AQCore.Shared.Items[itemData.name], "remove")
                        TriggerEvent('AQCore:Notify', 'Reloaded', "success")
                    end
                end, function()
                    AQCore.Functions.Notify("Canceled", "error")
                end)
            else
                AQCore.Functions.Notify("Max Ammo Capacity", "error")
            end
        else
            AQCore.Functions.Notify("You Have No Weapon", "error")
        end
    else
        AQCore.Functions.Notify("You Have No Weapon.", "error")
    end
...
AddEventHandler('AQCore:Client:OnPlayerLoaded', function()
    ...
    AQCore.Functions.TriggerCallback("weapons:server:GetConfig", function(RepairPoints)
                ...
                if distance < 10 then
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            ...
                            if CurrentWeaponData ~= nil and next(CurrentWeaponData) ~= nil then
                                if not data.RepairingData.Ready then
                                    local WeaponData = AQCore.Shared.Weapons[GetHashKey(CurrentWeaponData.name)]
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        AQCore.Functions.TriggerCallback('weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
...
AddEventHandler("weapons:client:EquipAttachment", function(ItemData, attachment)
    local ped = PlayerPedId()
    local weapon = GetSelectedPedWeapon(ped)
    local WeaponData = AQCore.Shared.Weapons[weapon]
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if WeaponAttachments[WeaponData.name] ~= nil then
            if WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                AQCore.Functions.Notify("This weapon does not support this attachment..", "error")
            end
        end
    else
        AQCore.Functions.Notify("You dont have a weapon in ur hand..", "error")
    end
end)
