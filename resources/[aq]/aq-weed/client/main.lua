...
RegisterNetEvent('aq-weed:client:getHousePlants')
AddEventHandler('aq-weed:client:getHousePlants', function(house)    
    AQCore.Functions.TriggerCallback('aq-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                ...
                for k, v in pairs(housePlants[currentHouse]) do
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    AQCore.Functions.Progressbar("remove_weed_plant", "Harvesting Plant", 8000, false, true, {
                                        ...
                                        TriggerServerEvent('aq-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                AQCore.Functions.Progressbar("remove_weed_plant", "Removing The Plant", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('aq-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
                                    AQCore.Functions.Notify("Process Canceled", "error")
                                end)
...
RegisterNetEvent('aq-weed:client:leaveHouse')
AddEventHandler('aq-weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('aq-weed:client:refreshHousePlants')
AddEventHandler('aq-weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('aq-weed:client:refreshPlantStats')
AddEventHandler('aq-weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(500, function()
            AQCore.Functions.TriggerCallback('aq-weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('aq-weed:client:placePlant')
AddEventHandler('aq-weed:client:placePlant', function(type, item)
    local ped = PlayerPedId()
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            AQCore.Functions.Progressbar("plant_weed_plant", "Planting", 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                ClearPedTasks(ped)            
                TriggerServerEvent('aq-weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
                TriggerServerEvent('aq-weed:server:removeSeed', item.slot, type)
            end, function() -- Cancel
                ClearPedTasks(ped)
                AQCore.Functions.Notify("Process Canceled", "error")
            end)
        else
            AQCore.Functions.Notify("Can't Place Here", 'error', 3500)
        end
    else
        AQCore.Functions.Notify("It's Not Safe Here, try your house", 'error', 3500)
    end
end)

RegisterNetEvent('aq-weed:client:foodPlant')
AddEventHandler('aq-weed:client:foodPlant', function(item)
    local plantData = {}
    if currentHouse ~= nil then
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    AQCore.Functions.Notify('The Plant Does Not Need Nutrition', 'error', 3500)
                else
                    AQCore.Functions.Progressbar("plant_weed_plant", "Feeding Plant", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        ...
                        TriggerServerEvent('aq-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                    end, function() -- Cancel
                        ClearPedTasks(ped)
                        AQCore.Functions.Notify("Process Canceled", "error")
                    end)
                ...
                AQCore.Functions.Notify("Can't Place Here", "error")
            end
        else
            AQCore.Functions.Notify("Can't Place Here", "error")
        end
